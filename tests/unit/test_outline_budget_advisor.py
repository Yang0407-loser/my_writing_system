...
def _contract(sub=None, names=None):
    return OutlineEventContractCompiler().compile_chapter(
        section=1,
        subsections=[sub or _sub()],
        character_names=names or ["林晚", "周野"],
        chapter_target_words=(sub or _sub()).get("target_words", 600),
    ).subsection_contracts[0]


def _advice(sub=None, style=None, names=None, brief=""):
    return OutlineBudgetAdvisor().advise_subsection(
        contract=_contract(sub, names),
        style_profile=style or {},
        style_brief=brief,
    )
...
def test_key_points_are_stable_and_interaction_chain_stays_whole():
    ...
    first = advisor.extract_units(**kwargs)
    second = advisor.extract_units(**kwargs)
    assert first == second
    assert len(first) == 1
    assert first[0].unit_type == "dialogue_interaction"
    assert first[0].actors == ("林晚", "周野")


def test_description_fallback_is_low_confidence():
    ...
    assert len(units) == 3
    assert {unit.confidence for unit in units} == {"low"}
    assert all(":description:" in unit.source_id for unit in units)


def test_specific_time_anchors_are_not_double_counted():
    advice = _advice(_sub(key_points=[
        "第一个周六林晚只听见声音",
        "第二个周六林晚看见周野的背影",
        "第三个周六林晚和周野在面包店门口相遇",
    ]))
...
def test_only_four_current_style_knobs_affect_formula():
    ...
    legacy = _advice(style={
        "paragraph_length_avg": 900,
        "narrative_density": 1.0,
    })
...
def test_paragraph_length_brief_is_conflict_only_not_budget_input():
    plain = _advice()
    conflict = _advice(brief="每段保持在四百字左右。")
...
def test_target_above_range_requests_review_instead_of_claiming_keep():
    advice = _advice(_sub(target_words=5000))
...
def test_overconstrained_chapter_recommends_scope_reduction():
    result = OutlineBudgetAdvisor().advise_outline(
        outline=[{"section": 1, "subsections": [
            ...
            _sub(subsection=2, source_id="b", target_words=200),
        ]}],
        character_names=["林晚", "周野"],
        chapter_budget=300,
    )
...
def test_advisor_consumes_contract_without_reinterpreting_raw_outline():
    compiler = OutlineEventContractCompiler()
    contract = compiler.compile_chapter(
        section=1,
        subsections=[_sub()],
        character_names=["林晚", "周野"],
    ...
    advisor = OutlineBudgetAdvisor()
    advisor.compiler.adapter.extract = lambda **_: (_ for _ in ()).throw(
        AssertionError("advisor must not extract after receiving a contract")
    )
    advice = advisor.advise_subsection(contract=contract, style_profile={})
    assert advice.event_units == list(contract.events)
