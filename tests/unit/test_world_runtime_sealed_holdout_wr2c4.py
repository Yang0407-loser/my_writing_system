...
from experiments.world_runtime_writer_canary import sealed_holdout_wr2c4 as holdout
...
def _synthetic_holdout(tmp_path):
    """Derive a non-reused synthetic holdout from the v4 fixture by appending a suffix."""
    ...
    payload = {
        "schema_version": "world-runtime-sealed-holdout-wr2c4-v1",
...
def _authorization(tmp_path, monkeypatch):
...
def test_seal_build_preflight_and_authorization(tmp_path, monkeypatch):
    _wire(tmp_path, monkeypatch)
    lock = holdout.seal(tmp_path)
    assert lock["holdout_sha256"] == hashlib.sha256(holdout.HOLDOUT_PATH.read_bytes()).hexdigest()
    assert lock["coverage"]["sample_count"] >= 20
    assert lock["coverage"]["all_13_types_covered"] is True

    manifest = holdout.build(tmp_path)
    assert manifest["partition_role"] == "sealed_unseen_holdout"
    assert manifest["sample_count"] == lock["coverage"]["sample_count"]

    missing = holdout.preflight(tmp_path, tmp_path / "missing.json")
    assert missing["ready"] is False

    authorization_path = tmp_path / "authorization.json"
    _write(authorization_path, _authorization(tmp_path, monkeypatch))
    ready = holdout.preflight(tmp_path, authorization_path)
    assert ready["ready"] is True
    assert ready["sealed_holdout_used"] is True
...
def test_fake_run_once_consumes_all_fails_closed_and_cannot_repeat(tmp_path, monkeypatch):
    ...
    assert result["state_commit_canary_authorized"] is False
