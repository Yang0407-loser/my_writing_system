...
from experiments.world_runtime_writer_canary import sealed_holdout_wr2c513r2 as holdout
...
def _synthetic_holdout(tmp_path):
    ...
    payload = {
        "schema_version": "world-runtime-sealed-holdout-wr2c513r2-v1",
...
def test_seal_accepts_validator_consistent_gold(tmp_path, monkeypatch):
    ...
    assert "r2" in lock["experiment_id"]
...
def test_seal_rejects_reuse_of_failed_513_holdout_text(tmp_path, monkeypatch):
    holdout_path, _ = _wire(tmp_path, monkeypatch)
    payload = _read(holdout_path)
    if holdout.FAILED_513_HOLDOUT.exists():
        failed = _read(holdout.FAILED_513_HOLDOUT)
        payload["cases"][0]["text"] = failed["cases"][0]["text"]
        _write(holdout_path, payload)
        with pytest.raises(ValueError, match="reuses a development/holdout text"):
            holdout.seal(tmp_path)
    else:
        pytest.skip("failed 513 holdout artifact absent")
