...
from experiments.world_runtime_writer_canary import sealed_holdout_wr2c513r5 as holdout
...
def _synthetic_holdout(tmp_path):
    ...
    payload = {
        "schema_version": "world-runtime-sealed-holdout-wr2c513r5-v1",
...
def test_runner_is_bound_to_r4_components():
    ...
    assert "r5" in holdout.RUNTIME.name
...
def test_seal_accepts_validator_consistent_gold(tmp_path, monkeypatch):
    ...
    assert "r5" in lock["experiment_id"]
...
def test_seal_rejects_reuse_of_previous_sealed_holdout_text(tmp_path, monkeypatch):
    holdout_path, _ = _wire(tmp_path, monkeypatch)
    payload = _read(holdout_path)
    prior_path = holdout.WR2C513R4_SEALED_HOLDOUT
    if prior_path.exists():
        ...
        pytest.skip("previous sealed holdout artifact absent")
