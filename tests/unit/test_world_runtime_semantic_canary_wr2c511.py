...
from experiments.world_runtime_writer_canary import external_runner_wr2c511
...
def _authorization(output_dir):
    ...
    return {
        ...
        "external_runner_sha256": hashlib.sha256(external_runner_wr2c511.SOURCE.read_bytes()).hexdigest(),
...
def test_build_freezes_development_v511(tmp_path):
    ...
    assert manifest["partition_role"] == "visible_development_v511_not_holdout"
...
def test_hash_bound_authorization(tmp_path):
    ...
    missing = external_runner_wr2c511.preflight(tmp_path, tmp_path / "missing.json")
    assert missing["ready"] is False
    authorization = _authorization(tmp_path)
    path = tmp_path / "authorization.json"
    _write(path, authorization)
    ready = external_runner_wr2c511.preflight(tmp_path, path)
...
def test_fake_run_once_fails_closed_and_cannot_repeat(tmp_path, monkeypatch):
    ...
    fake = _FakeClient()
    monkeypatch.setattr(external_runner_wr2c511, "get_llm_client", lambda model: fake)
    result = external_runner_wr2c511.run_once(tmp_path, authorization_path)
    ...
    with pytest.raises(RuntimeError, match="external_preflight_failed"):
        external_runner_wr2c511.run_once(tmp_path, authorization_path)
