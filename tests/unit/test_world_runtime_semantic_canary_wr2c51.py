...
from experiments.world_runtime_writer_canary import external_runner_wr2c51
from experiments.world_runtime_writer_canary import semantic_canary_wr2c51 as canary
...
def _authorization(output_dir):
    ...
    return {
        ...
        "external_runner_sha256": hashlib.sha256(external_runner_wr2c51.SOURCE.read_bytes()).hexdigest(),
...
def test_hash_bound_authorization(tmp_path):
    ...
    missing = external_runner_wr2c51.preflight(tmp_path, tmp_path / "missing.json")
    assert missing["ready"] is False
    authorization = _authorization(tmp_path)
    path = tmp_path / "authorization.json"
    _write(path, authorization)
    ready = external_runner_wr2c51.preflight(tmp_path, path)
...
def test_fake_run_once_fails_closed_and_cannot_repeat(tmp_path, monkeypatch):
    ...
    fake = _FakeClient()
    monkeypatch.setattr(external_runner_wr2c51, "get_llm_client", lambda model: fake)
    result = external_runner_wr2c51.run_once(tmp_path, authorization_path)
    ...
    with pytest.raises(RuntimeError, match="external_preflight_failed"):
        external_runner_wr2c51.run_once(tmp_path, authorization_path)
