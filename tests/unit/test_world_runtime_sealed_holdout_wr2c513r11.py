...
from experiments.world_runtime_writer_canary import sealed_holdout_wr2c513r11 as holdout
...
def _synthetic_holdout(tmp_path):
    ...
    payload = {
        "schema_version": "world-runtime-sealed-holdout-wr2c513r11-v1",
...
def test_seal_accepts_corpus_with_shifted_day_case(tmp_path, monkeypatch):
    ...
    assert "r11" in lock["experiment_id"]
    manifest = holdout.build(tmp_path)
    assert manifest["sample_count"] == lock["coverage"]["sample_count"]
    assert manifest["samples"][0]["sample_id"].startswith("WR2C513R11H-")
...
def test_seal_rejects_reuse_of_r10_holdout_text(tmp_path, monkeypatch):
    holdout_path, _ = _wire(tmp_path, monkeypatch)
    payload = _read(holdout_path)
    prior_path = holdout.WR2C513R10_SEALED_HOLDOUT
    if prior_path.exists():
        ...
        pytest.skip("r10 sealed holdout artifact absent")
