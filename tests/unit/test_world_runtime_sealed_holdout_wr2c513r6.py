...
from experiments.world_runtime_writer_canary import sealed_holdout_wr2c513r6 as holdout
...
def _synthetic_holdout(tmp_path, with_shifted_day=True):
    ...
    payload = {
        "schema_version": "world-runtime-sealed-holdout-wr2c513r6-v1",
...
def _wire(tmp_path, monkeypatch, with_shifted_day=True):
    holdout_path = _synthetic_holdout(tmp_path, with_shifted_day=with_shifted_day)
...
def test_runner_is_bound_to_r4_components():
    assert holdout.EXTRACTOR.name == "semantic_extractor_wr2c513r4.py"
    assert holdout.PROJECTOR.name == "semantic_projector_wr2c513r4.py"
    assert "r6" in holdout.RUNTIME.name
...
def test_seal_accepts_corpus_with_shifted_day_case(tmp_path, monkeypatch):
    ...
    assert "r6" in lock["experiment_id"]
    manifest = holdout.build(tmp_path)
    assert manifest["sample_count"] == lock["coverage"]["sample_count"]
    assert manifest["samples"][0]["sample_id"].startswith("WR2C513R6H-")
...
def test_seal_rejects_corpus_without_shifted_day_case(tmp_path, monkeypatch):
    _wire(tmp_path, monkeypatch, with_shifted_day=False)
    with pytest.raises(ValueError, match="past/future-day time reference case"):
        holdout.seal(tmp_path)


def test_seal_rejects_reuse_of_r5_holdout_text(tmp_path, monkeypatch):
    holdout_path, _ = _wire(tmp_path, monkeypatch)
    payload = _read(holdout_path)
    prior_path = holdout.WR2C513R5_SEALED_HOLDOUT
    if prior_path.exists():
        ...
        pytest.skip("r5 sealed holdout artifact absent")
