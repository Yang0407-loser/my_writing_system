...
from experiments.world_runtime_writer_canary import sealed_holdout_wr2c513r3 as holdout
...
def _synthetic_holdout(tmp_path):
    ...
    payload = {
        "schema_version": "world-runtime-sealed-holdout-wr2c513r3-v1",
...
def test_runner_is_bound_to_r3_components():
    assert holdout.EXTRACTOR.name == "semantic_extractor_wr2c513r3.py"
    assert holdout.PROJECTOR.name == "semantic_projector_wr2c513r3.py"
    old_extractor_path = holdout.ROOT / "experiments/world_runtime_writer_canary/semantic_extractor_wr2c513.py"
    old_projector_path = holdout.ROOT / "experiments/world_runtime_writer_canary/semantic_projector_wr2c513.py"
    assert hashlib.sha256(holdout.EXTRACTOR.read_bytes()).hexdigest() != hashlib.sha256(old_extractor_path.read_bytes()).hexdigest()
    assert hashlib.sha256(holdout.PROJECTOR.read_bytes()).hexdigest() != hashlib.sha256(old_projector_path.read_bytes()).hexdigest()
...
def test_seal_accepts_validator_consistent_gold(tmp_path, monkeypatch):
    ...
    assert "r3" in lock["experiment_id"]
...
def test_seal_rejects_reuse_of_failed_513_r2_holdout_text(tmp_path, monkeypatch):
    holdout_path, _ = _wire(tmp_path, monkeypatch)
    payload = _read(holdout_path)
    if holdout.FAILED_513R2_HOLDOUT.exists():
        failed = _read(holdout.FAILED_513R2_HOLDOUT)
        payload["cases"][0]["text"] = failed["cases"][0]["text"]
        _write(holdout_path, payload)
        with pytest.raises(ValueError, match="reuses a development/holdout text"):
            holdout.seal(tmp_path)
    else:
        pytest.skip("failed 513r2 holdout artifact absent")
