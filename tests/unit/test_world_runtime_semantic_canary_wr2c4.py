...
from experiments.world_runtime_writer_canary import external_runner_wr2c4
from experiments.world_runtime_writer_canary import semantic_canary_wr2c4 as canary
...
def _authorization(output_dir):
    ...
    return {
        "schema_version": "world-runtime-semantic-canary-wr2c4-external-authorization-v1",
        "authorized": True,
        "experiment_id": manifest["experiment_id"],
        "locked_manifest_sha256": hashlib.sha256(manifest_path.read_bytes()).hexdigest(),
        "external_runner_sha256": hashlib.sha256(external_runner_wr2c4.SOURCE.read_bytes()).hexdigest(),
...
def test_build_freezes_development_v4_without_old_or_holdout_reuse(tmp_path):
    ...
    assert manifest["partition_role"] == "visible_development_v4_not_holdout"
...
def test_hash_bound_authorization_blocks_old_partition_holdout_or_commit(tmp_path):
    ...
    missing = external_runner_wr2c4.preflight(tmp_path, tmp_path / "missing.json")
    assert missing["ready"] is False

    authorization = _authorization(tmp_path)
    path = tmp_path / "authorization.json"
    _write(path, authorization)
    ready = external_runner_wr2c4.preflight(tmp_path, path)
    ...
    authorization["sealed_holdout_use_authorized"] = True
    _write(path, authorization)
    blocked = external_runner_wr2c4.preflight(tmp_path, path)
    assert blocked["ready"] is False
    assert any("sealed_holdout_use_authorized" in issue for issue in blocked["issues"])
...
def test_fake_run_consumes_twenty_once_fails_closed_and_cannot_be_repeated(tmp_path, monkeypatch):
    ...
    fake = _FakeClient()
    monkeypatch.setattr(external_runner_wr2c4, "get_llm_client", lambda model: fake)

    result = external_runner_wr2c4.run_once(tmp_path, authorization_path)
    ...
    with pytest.raises(RuntimeError, match="external_preflight_failed"):
        external_runner_wr2c4.run_once(tmp_path, authorization_path)
