...
from experiments.world_runtime_writer_canary import sealed_holdout_wr2c513r7 as holdout
...
def _synthetic_holdout(tmp_path):
    ...
    payload = {
        "schema_version": "world-runtime-sealed-holdout-wr2c513r7-v1",
...
def test_runner_is_bound_to_chained_components():
    assert holdout.EXTRACTOR.name == "semantic_extractor_wr2c513r5.py"
    assert holdout.PROJECTOR.name == "semantic_projector_wr2c513r5.py"
    assert holdout.VALIDATOR.name == "delta_shadow_wr2c6.py"
    assert "r7" in holdout.RUNTIME.name
...
def test_seal_accepts_corpus_with_shifted_day_case(tmp_path, monkeypatch):
    ...
    assert "r7" in lock["experiment_id"]
    manifest = holdout.build(tmp_path)
    assert manifest["sample_count"] == lock["coverage"]["sample_count"]
    assert manifest["samples"][0]["sample_id"].startswith("WR2C513R7H-")
...
def test_seal_rejects_reuse_of_r6_holdout_text(tmp_path, monkeypatch):
    holdout_path, _ = _wire(tmp_path, monkeypatch)
    payload = _read(holdout_path)
    prior_path = holdout.WR2C513R6_SEALED_HOLDOUT
    if prior_path.exists():
        ...
        pytest.skip("r6 sealed holdout artifact absent")
