...
from experiments.world_runtime_writer_canary import sealed_holdout_wr2c513r12 as holdout
...
def _synthetic_holdout(tmp_path):
    ...
    judgments[_TYPES.index("clock_state")] = {
        ...
        "mode": "actual",
        "epistemic": "asserted",
        "evidence": [{"excerpt": "六点二十二分", "occurrence": 1}],
    }
    judgments[_TYPES.index("publication_state")] = {
        ...
        "epistemic": "asserted",
        "evidence": [
            {"excerpt": "林晚点下“发布”。", "occurrence": 1},
            {"excerpt": "页面刷新，文章出现在博客首页", "occurrence": 1},
        ],
    }
    cases.append({
        "case_id": "SYN-MULTIEV-01",
        "scene_id": "adversarial-cross-paragraph-evidence",
        "state_variant": "before",
        "text": "六点二十二分，林晚点下“发布”。\n\n页面刷新，文章出现在博客首页。",
        "judgments": judgments,
        "changes": [
    ...
    judgments[_TYPES.index("clock_state")] = {
        ...
        "mode": "actual",
        "epistemic": "asserted",
        "evidence": [{"excerpt": "六点二十三分", "occurrence": 1}],
    }
    judgments[_TYPES.index("knowledge_state")] = {
        "change_type": "knowledge_state",
        "occurred": True,
        ...
        "epistemic": "asserted",
        "evidence": [
            {"excerpt": "林晚把文章链接发进私聊", "occurrence": 1},
            {"excerpt": "季晴看完后引用了一句", "occurrence": 1},
        ],
    }
    cases.append({
        "case_id": "SYN-TRANSMIT-01",
        "scene_id": "adversarial-transmission-perception",
        "state_variant": "before",
        "text": "六点二十三分，林晚把文章链接发进私聊，季晴看完后引用了一句。",
        "judgments": judgments,
        "changes": [
            {
                "change_type": "clock_state",
            ...
            },
            {
                ...
                "predicate": "article_knowledge",
                "after_value": "perceived",
                "mechanism": "private_link_send_and_body_response",
                "expected_validation": "valid",
            },
        ],
    })
    ...
    judgments[_TYPES.index("employment_state")] = {
        "change_type": "employment_state",
        "occurred": True,
        "after_value": "ended",
        "mode": "actual",
        "epistemic": "asserted",
        "evidence": [{"excerpt": "自己做完这周就不来了", "occurrence": 1}],
    }
    cases.append({
        "case_id": "SYN-SELFASSUMED-01",
        "scene_id": "adversarial-self-assumed-employment",
        "state_variant": "before",
        "text": "六点十二分，林晚对周野说，自己做完这周就不来了。",
        "judgments": judgments,
        "changes": [
            {
                "change_type": "clock_state",
    ...
    payload = {
        "schema_version": "world-runtime-sealed-holdout-wr2c513r12-v1",
...
def test_seal_accepts_corpus_with_shifted_day_case(tmp_path, monkeypatch):
    ...
    assert "r12" in lock["experiment_id"]
    manifest = holdout.build(tmp_path)
    assert manifest["sample_count"] == lock["coverage"]["sample_count"]
    assert manifest["samples"][0]["sample_id"].startswith("WR2C513R12H-")
...
def test_seal_rejects_corpus_without_cross_paragraph_evidence_case(tmp_path, monkeypatch):
    holdout_path, _ = _wire(tmp_path, monkeypatch)
    payload = _read(holdout_path)
    payload["cases"] = [
        c for c in payload["cases"] if c["case_id"] != "SYN-MULTIEV-01"
    ]
    _write(holdout_path, payload)
    with pytest.raises(ValueError, match="multi-evidence cross-paragraph case"):
        holdout.seal(tmp_path)


def test_seal_rejects_corpus_without_transmission_perception_case(tmp_path, monkeypatch):
    holdout_path, _ = _wire(tmp_path, monkeypatch)
    payload = _read(holdout_path)
    payload["cases"] = [
        c for c in payload["cases"] if c["case_id"] != "SYN-TRANSMIT-01"
    ]
    _write(holdout_path, payload)
    with pytest.raises(ValueError, match="transmission\\+perception knowledge case"):
        holdout.seal(tmp_path)


def test_seal_rejects_corpus_without_self_assumed_employment_case(tmp_path, monkeypatch):
    holdout_path, _ = _wire(tmp_path, monkeypatch)
    payload = _read(holdout_path)
    payload["cases"] = [
        c for c in payload["cases"]
        if not any(
            ch["change_type"] == "employment_state"
            and ch.get("mechanism") == "self_assumed_effective"
            for ch in c.get("changes", [])
        )
    ]
    _write(holdout_path, payload)
    with pytest.raises(ValueError, match="self-assumed employment case"):
        holdout.seal(tmp_path)


def test_seal_rejects_reuse_of_r11_holdout_text(tmp_path, monkeypatch):
    holdout_path, _ = _wire(tmp_path, monkeypatch)
    payload = _read(holdout_path)
    prior_path = holdout.WR2C513R11_SEALED_HOLDOUT
    if prior_path.exists():
        ...
        pytest.skip("r11 sealed holdout artifact absent")
