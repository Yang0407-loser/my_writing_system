...
from experiments.world_runtime_writer_canary import state_commit_canary_wr2c513r11 as canary
...
def test_runner_is_bound_to_chained_components():
    assert canary.EXTRACTOR.name == "semantic_extractor_wr2c513r10.py"
    assert canary.PROJECTOR.name == "semantic_projector_wr2c513r6.py"
    assert canary.VALIDATOR.name == "delta_shadow_wr2c6.py"
    assert canary.ADAPTER.name == "state_commit_adapter_wr2c7.py"
    assert "c21r10" in canary.RUNTIME.name


def test_build_and_preflight_with_authorization(tmp_path):
    manifest = canary.build(tmp_path)
    assert manifest["subsection_count"] == 3
    assert "c21r10" in manifest["experiment_id"]
...
class _TwoClockFakeClient:
    def __init__(self):
        self.calls = 0
        self.texts = [
            "六点整，林晚打开店门开始营业。",
            "六点五分，周野看了一眼挂钟。",
            "六点零七分，周野看了一眼挂钟；六点二十二分，他又看了一眼。",
        ]

    def _judgments(self, index):
        judgments = [
            {
                "change_type": change_type,
                "occurred": False,
        ...
        specs = [
            [("clock_state", "06:00", "六点整"), ("storefront_operation_state", "open", "打开店门开始营业")],
            [("clock_state", "06:05", "六点五分")],
            [("clock_state", "06:07", "六点零七分"), ("clock_state", "06:22", "六点二十二分")],
        ]
        for change_type, after_value, excerpt in specs[index]:
            judgments[_TYPES.index(change_type)] = {
                "change_type": change_type,
                "occurred": True,
                "after_value": after_value,
    ...
    def chat_completion(self, messages, **kwargs):
        ...
        index = (self.calls - 1) // 2
        if kwargs["json_mode"]:
            return json.dumps({"judgments": self._judgments(index)}, ensure_ascii=False)
        return self.texts[index]


def test_fake_run_once_two_clocks_in_one_subsection_commit(tmp_path, monkeypatch):
    canary.build(tmp_path)
    authorization_path = tmp_path / "authorization.json"
    _write(authorization_path, _authorization(tmp_path))
    fake = _TwoClockFakeClient()
    ...
    s3 = _read(tmp_path / "private/commits/S3.json")
    clock_entries = [
        entry for entry in s3["ledger"]["entries"]
        if entry["change_type"] == "clock_state"
    ]
    assert [entry["after_value"] for entry in clock_entries] == ["06:07", "06:22"]
