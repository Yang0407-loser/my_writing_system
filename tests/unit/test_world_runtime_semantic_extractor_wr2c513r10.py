...
def test_prompt_contains_impossible_perception_worked_example():
    ...
    assert "WORKED EXAMPLE 5" in prompt
    assert "transmission segment" in prompt
    assert "WORKED EXAMPLE 6" in prompt
    assert "self_assumed_effective" in prompt
