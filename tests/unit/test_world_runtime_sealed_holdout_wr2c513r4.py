...
from experiments.world_runtime_writer_canary import sealed_holdout_wr2c513r4 as holdout
...
def _synthetic_holdout(tmp_path):
    ...
    payload = {
        "schema_version": "world-runtime-sealed-holdout-wr2c513r4-v1",
...
def test_runner_is_bound_to_r3_components():
    ...
    assert "r4" in holdout.RUNTIME.name
...
def test_seal_accepts_validator_consistent_gold(tmp_path, monkeypatch):
    ...
    assert "r4" in lock["experiment_id"]
...
def test_seal_rejects_reuse_of_failed_513r3_holdout_text(tmp_path, monkeypatch):
    holdout_path, _ = _wire(tmp_path, monkeypatch)
    payload = _read(holdout_path)
    if holdout.FAILED_513R3_HOLDOUT.exists():
        failed = _read(holdout.FAILED_513R3_HOLDOUT)
        payload["cases"][0]["text"] = failed["cases"][0]["text"]
        _write(holdout_path, payload)
        with pytest.raises(ValueError, match="reuses a development/holdout text"):
            holdout.seal(tmp_path)
    else:
        pytest.skip("failed 513r3 holdout artifact absent")
