...
import * as API from './api.js?v=20260822d';
  ...
  return createApp({
    setup() {
      ...
      function beginPolling(flatOutline, keepExisting=false, options={manualRetry:false,deferStream:false}) {
        ...
        let stopped=false;
        const eventLedger = createStreamEventLedger();
        function stop(){stopped=true;eventLedger.close();stopPolling()}
        ...
        async function pstr(){if(stopped||!pollingTaskId||!session.isActive()||activePollingSession!==session)return;if(!streamGate.isReady()||!controller.snapshot().shouldPollStream){pollTimers.stream=setTimeout(pstr,15000);return}try{const d=await API.getStream(pollingTaskId,streamGate.cursor(),{signal:session.signal});if(!session.isActive()||activePollingSession!==session)return;noteSuccess('stream');streamGate.recordCursor(d.last_id);for(const[eventId,evt]of(d.events||[])){if(!eventLedger.accept(eventId,evt))continue;if(stopped||!session.isActive()||activePollingSession!==session)break;if(evt.event==='section_start'||evt==='section_start'){const i=findBlock(evt.section,evt.subsection);if(i>=0){generatingBlockIdx.value=i;draftBlocks.value[i].text=''}}else if(evt.event==='token'||evt==='token'){const i=findBlock(evt.section,evt.subsection);if(i>=0){draftBlocks.value[i].text+=(evt.token||evt.data||'');draftBlocks.value[i].wordCount=countCjk(draftBlocks.value[i].text)}}else if(evt.event==='section_end'||evt==='section_end'||evt.event==='canonical_subsection_committed'){const i=findBlock(evt.section,evt.subsection);if(i>=0){draftBlocks.value[i].text=evt.text||evt.data||evt.text||evt.data||draftBlocks.value[i].text;draftBlocks.value[i].wordCount=countCjk(draftBlocks.value[i].text)}markLeafStatus(evt.section,evt.subsection,'done');generatingBlockIdx.value=-1;completedSections.value=Math.max(completedSections.value,evt.section||0);if(evt.event==='canonical_subsection_committed'){commitStatus.value='committed';projectionStatus.value='non_blocking'}}else if(evt.event==='done'||evt==='done'){stop();taskDone.value=true;saveState();loadForeshadowings();loadCharacters();loadFactions();loadMap();loadItems();loadSubplots();loadRelations();showCompleteModal.value=true;loadReviewResults();return}}if(d.status==='completed'||d.status==='failed'){stop();return}}catch(e){if(!session.isActive()||activePollingSession!==session)return;noteFailure('stream',e)}if(!stopped&&session.isActive()&&activePollingSession===session)pollTimers.stream=setTimeout(pstr,controller.failureCount('stream')?retryDelay(controller.failureCount('stream'),300):300)}
