...
from experiments.world_runtime_writer_canary.semantic_projector_wr2c511 import project


ROOT = Path(__file__).resolve().parents[2]
SOURCE = Path(__file__).resolve()
EXTRACTOR = ROOT / "experiments/world_runtime_writer_canary/semantic_extractor_wr2c511.py"
PROJECTOR = ROOT / "experiments/world_runtime_writer_canary/semantic_projector_wr2c511.py"
VALIDATOR = ROOT / "experiments/world_runtime_writer_canary/delta_shadow_wr2c5.py"
RUNTIME = ROOT / ".world_runtime_wr2c511_sealed_holdout_runtime"
...
def seal(output_dir: Path = RUNTIME) -> dict[str, Any]:
    ...
    lock = {
        "schema_version": "world-runtime-sealed-holdout-wr2c511-lock-v1",
        "experiment_id": "world-runtime-semantic-judgment-wr2c511-sealed-unseen-holdout",
...
def build(output_dir: Path = RUNTIME) -> dict[str, Any]:
    ...
    for ordinal, case in enumerate(cases, 1):
        ...
        sample_id = f"WR2C511H-{ordinal:02d}"
        samples.append({
            "sample_id": sample_id,
    ...
    manifest = {
        "schema_version": "world-runtime-sealed-holdout-wr2c511-manifest-v1",
        "experiment_id": "world-runtime-semantic-judgment-wr2c511-sealed-unseen-holdout",
        "partition_role": "sealed_unseen_holdout",
        "holdout_sha256": _sha256_file(HOLDOUT_PATH),
    ...
    _write_json(manifest_path, manifest)
    ledger = {
        "schema_version": "world-runtime-sealed-holdout-wr2c511-ledger-v1",
        "samples": {
            sample["sample_id"]: {
...
def audit(output_dir: Path = RUNTIME) -> dict[str, Any]:
    ...
    return {
        "schema_version": "world-runtime-sealed-holdout-wr2c511-preflight-v1",
        "ready": not issues,
        "issues": issues,
...
def preflight(
    ...
    expected = {
        "schema_version": "world-runtime-semantic-canary-wr2c511-sealed-holdout-external-authorization-v1",
        "authorized": True,
    ...
    return {
        "schema_version": "world-runtime-sealed-holdout-wr2c511-external-preflight-v1",
        "ready": not issues,
        "issues": issues,
...
def evaluate(output_dir: Path = RUNTIME) -> dict[str, Any]:
    ...
    result = {
        "schema_version": "world-runtime-sealed-holdout-wr2c511-evaluation-v1",
        "partition_role": "sealed_unseen_holdout",
        "sample_count": manifest["sample_count"],
...
def run_once(
    ...
    for sample in manifest["samples"]:
        ...
        try:
            response = client.chat_completion(
                ...
                prompt_name="world_runtime_semantic_judgment_wr2c511_holdout_v1",
    ...
    return {
        "schema_version": "world-runtime-sealed-holdout-wr2c511-external-result-v1",
