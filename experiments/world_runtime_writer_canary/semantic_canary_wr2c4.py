...
FIXTURE = ROOT / "experiments/world_runtime_writer_canary/fixtures/wr2c4_semantic_development_v4.json"
EXTERNAL_RUNNER = ROOT / "experiments/world_runtime_writer_canary/external_runner_wr2c4.py"
DEFAULT_OUTPUT = ROOT / ".world_runtime_wr2c4_semantic_canary_runtime"
...
def build(output_dir: Path = DEFAULT_OUTPUT) -> dict[str, Any]:
    ...
    manifest = {
        "schema_version": "world-runtime-semantic-canary-wr2c4-manifest-v1",
        "experiment_id": "world-runtime-semantic-judgment-wr2c4-development-v4",
        "partition_role": "visible_development_v4_not_holdout",
    ...
    ledger = {
        "schema_version": "world-runtime-semantic-canary-wr2c4-ledger-v1",
        "samples": {
            sample["sample_id"]: {
...
def _assert_integrity(output_dir: Path) -> dict[str, Any]:
    ...
    for field, expected in checks.items():
        if manifest.get(field) != expected:
            raise RuntimeError(f"wr2c4_frozen_drift:{field}")
    return manifest
...
def audit(output_dir: Path = DEFAULT_OUTPUT) -> dict[str, Any]:
    ...
    result = {
        "schema_version": "world-runtime-semantic-canary-wr2c4-preflight-v1",
...
def evaluate(output_dir: Path = DEFAULT_OUTPUT) -> dict[str, Any]:
    ...
    result = {
        "schema_version": "world-runtime-semantic-canary-wr2c4-evaluation-v1",
        "partition_role": "visible_development_v4_not_holdout",
        "sample_count": manifest["sample_count"],
        ...
        "state_mutations": 0,
        "commits": 0,
        "decision": "development_v4_passed_new_unseen_holdout_required" if passed else "hold_semantic_development_v4_failed",
        "items": items,
    }
