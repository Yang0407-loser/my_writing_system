"""One-shot external executor for the frozen WR2-C2 Development-v2 canary."""
...
from experiments.world_runtime_writer_canary import semantic_canary_wr2c2 as wr2c2


SOURCE = Path(__file__).resolve()
DEFAULT_AUTHORIZATION = wr2c2.DEFAULT_OUTPUT / "private/external-execution-authorization.json"
...
def preflight(
    output_dir: Path = wr2c2.DEFAULT_OUTPUT,
    authorization_path: Path = DEFAULT_AUTHORIZATION,
) -> dict[str, Any]:
    audit = wr2c2.audit(output_dir)
    ...
    return {
        "schema_version": "world-runtime-semantic-canary-wr2c2-external-preflight-v1",
        "ready": not issues,
        "issues": issues,
...
def run_once(
    ...
    if not check["ready"]:
        raise RuntimeError("wr2c2_external_preflight_failed:" + "|".join(check["issues"]))
    ...
    for sample in manifest["samples"]:
        ...
        if entry["status"] != "pending" or entry["attempt_count"] != 0:
            raise RuntimeError(f"wr2c2_refusing_nonpristine_sample:{sample['sample_id']}")
        ...
        try:
            response = client.chat_completion(
                ...
                prompt_name="world_runtime_semantic_extractor_wr2c2_v1",
            ...
            raise
        _write(ledger_path, ledger)
    evaluation = wr2c2.evaluate(output_dir)
    return {
        "schema_version": "world-runtime-semantic-canary-wr2c2-external-result-v1",
...
def main() -> None:
    ...
    parser.add_argument("--output", type=Path, default=wr2c2.DEFAULT_OUTPUT)
