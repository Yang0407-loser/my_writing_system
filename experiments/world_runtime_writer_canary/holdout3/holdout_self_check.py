# -*- coding: utf-8 -*-
"""holdout3 gold self-check: schema, hashes, verbatim spans, prior-context, coverage, seal."""
import hashlib
import json
import os
import sys
...
check("schema_version", gold.get("schema_version") == "wr4-metadata-holdout-gold-v1", gold.get("schema_version"))
check("author", gold.get("author") == "independent-holdout-author-human-batch3", gold.get("author"))
...
check("query_index H1..H14", [e["query_index"] for e in entries] == [f"H{i}" for i in range(1, 15)])
...
# Writing point coverage: 前段 2-6 / 中段 7-13 / 后段 14-18
...
for e in entries:
    ...
    # gold_chunk_hashes == all chunks in gold_sections
    expected_chunks = set()
    for s in e["gold_sections"]:
...
check("seal author", seal.get("author") == "independent-holdout-author-human-batch3")
