"""WR2-C5 semantic judgment contract and parser.

Same judgment+evidence contract as WR2-C4, with two changes:

1. New type 14 ``storefront_operation_state`` for explicit store opening/closing.
2. Type 12 counts any explicit narrative time progression (wall clock, screen
   clock, "六点钟", "五点三十一分") as clock_state.

Canonical fields remain fully owned by the WR2-C5 projector.
"""
...
from experiments.world_runtime_writer_canary.semantic_projector_wr2c5 import (
    build_delta,
    project,
)


SEMANTIC_EXTRACTOR_VERSION = "world-runtime-semantic-extractor-wr2c5-v1"
MAX_JUDGMENTS = 32
...
class SemanticExtractionArtifactC5(FrozenModel):
...
def parse_semantic_response(
    *,
    ...
    try:
        ...
        raise ValueError("wr2c5_invalid_judgment_response") from exc
    ...
    if delta.output_hash != output_hash:
        raise RuntimeError("wr2c5_output_hash_mismatch")
    return SemanticExtractionArtifactC5(
        sample_id=sample_id,
