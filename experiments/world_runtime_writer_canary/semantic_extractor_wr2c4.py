"""WR2-C4 semantic judgment contract and parser.

Same judgment+evidence contract as WR2-C3, with two surgical prompt changes:

1. Type 8 explicitly states that a character's own declaration/assumption of
   employment termination still counts as occurred=true (legality is local).
2. Type 6 requires a single state word and forbids composite values such as
   ``empty_and_restored``.

Canonical fields remain fully owned by the WR2-C4 projector.
"""
...
from experiments.world_runtime_writer_canary.semantic_projector_wr2c4 import (
    build_delta,
    project,
)


SEMANTIC_EXTRACTOR_VERSION = "world-runtime-semantic-extractor-wr2c4-v1"
MAX_JUDGMENTS = 32
...
class SemanticExtractionArtifactC4(FrozenModel):
...
8. Return all 13 judgments even when no durable change occurs.
"""
...
def parse_semantic_response(
    *,
    ...
    try:
        ...
        raise ValueError("wr2c4_invalid_judgment_response") from exc
    ...
    if delta.output_hash != output_hash:
        raise RuntimeError("wr2c4_output_hash_mismatch")
    return SemanticExtractionArtifactC4(
        sample_id=sample_id,
