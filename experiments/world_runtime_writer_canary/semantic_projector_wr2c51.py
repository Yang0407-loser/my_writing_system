"""WR2-C5.1 deterministic projector.

Same boundary as WR2-C5 (model judges, projector owns canonical fields) plus:

1. Dependency ordering: clock -> storefront_operation -> sale/handoff (the
   Validator sees the chained state in the correct order);
2. Sale-subsumes-handoff dedup: in a delta that contains a sale, non-free
   handoffs of the same storefront action are treated as sale implementation
   details and dropped.
...
PROJECTOR_VERSION = "world-runtime-projector-wr2c51-v1"
    ...
    *,
    ...
    for sequence, event in enumerate(events, 1):
        ...
        for evidence_index, (evidence, span) in enumerate(event.evidence, 1):
            evidence_id = f"ev:wr2c51:{sample_id.lower()}:{event.judgment_index + 1}:{evidence_index}"
        ...
        changes.append(ProposedChangeV5(
            change_id=f"change:wr2c51:{sample_id.lower()}:{sequence}",
            sequence=sequence,
            change_type=event.change_type,
    ...
    return ProposedTypedDeltaV5(
        delta_id=f"delta:wr2c51:{sample_id.lower()}",
        sample_id=sample_id,
        scene_id=scene_id,
