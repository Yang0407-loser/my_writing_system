"""WR2-C5.1.3-R4 State Commit Canary Phase C2.1-R7: limited real-generation canary.
...
RUNTIME = ROOT / ".world_runtime_state_commit_canary_runtime" / "c21r7"
...
def _assert_component_binding() -> None:
    if SOURCE.name != "state_commit_canary_wr2c513r8.py":
...
def build(output_dir: Path = RUNTIME) -> dict[str, Any]:
    ...
    manifest = {
        "schema_version": "world-runtime-state-commit-canary-c21r7-manifest-v1",
        "experiment_id": "world-runtime-state-commit-canary-c21r7",
    ...
    ledger = {
        "schema_version": "world-runtime-state-commit-canary-c21r7-ledger-v1",
        "samples": {
            sample["subsection"]: {
...
def audit(output_dir: Path = RUNTIME) -> dict[str, Any]:
    ...
    return {
        "schema_version": "world-runtime-state-commit-canary-c21r7-preflight-v1",
        "ready": not issues,
        "issues": issues,
        "pending": pending,
...
def preflight(
    ...
    expected = {
        "schema_version": "world-runtime-state-commit-canary-c21r7-external-authorization-v1",
        "authorized": True,
    ...
    return {
        "schema_version": "world-runtime-state-commit-canary-c21r7-external-preflight-v1",
        "ready": not issues,
        "issues": issues,
...
def run_once(
    ...
    if not check["ready"]:
        raise RuntimeError("c21r7_preflight_failed:" + "|".join(check["issues"]))
    manifest = _read_json(output_dir / "private/locked-manifest.json")
    ledger_path = output_dir / "attempt-ledger.json"
    ...
    for sample in manifest["samples"]:
        ...
        if entry["status"] != "pending" or entry["attempt_count"] != 0:
            raise RuntimeError(f"c21r7_refusing_nonpristine_subsection:{subsection}")
        ...
        try:
            ...
            final_text = client.chat_completion(
                ...
                prompt_name="state_commit_canary_c21r7_generation_v1",
            ...
            judgment = client.chat_completion(
                ...
                prompt_name="state_commit_canary_c21r7_judgment_v1",
            ...
            artifact = parse_semantic_response(
                ...
                sample_id=f"C21R7-S{subsection}",
    ...
    report = {
        "schema_version": "world-runtime-state-commit-canary-c21r7-report-v1",
        "partition_role": "limited_real_canary",
        "summary": summary,
        "records": records,
