"""WR2-C5.1.3-R3 deterministic projector.

Same boundary as WR2-C5.1.3, with one surgical fix: the ``actor_present`` check
in object resolution no longer requires a pour action, so an explicit
cleaned-and-stored action with a unique full-text actor projects as
``explicit_action`` instead of ``missing_actor_or_event``.

Clock time ordering, repeat subsumption, open fallback, multi-clock fallback
and composite split are unchanged.
...
PROJECTOR_VERSION = "world-runtime-projector-wr2c513r3-v1"
    ...
    *,
    ...
    for sequence, event in enumerate(events, 1):
        ...
        for evidence_index, (evidence, span) in enumerate(event.evidence, 1):
            evidence_id = f"ev:wr2c513r3:{sample_id.lower()}:{sequence}:{evidence_index}"
        ...
        changes.append(ProposedChangeV5(
            change_id=f"change:wr2c513r3:{sample_id.lower()}:{sequence}",
            sequence=sequence,
            change_type=event.change_type,
    ...
    return ProposedTypedDeltaV5(
        delta_id=f"delta:wr2c513r3:{sample_id.lower()}",
        sample_id=sample_id,
        scene_id=scene_id,
