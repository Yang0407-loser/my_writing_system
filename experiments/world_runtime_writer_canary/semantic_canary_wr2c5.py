...
from experiments.world_runtime_writer_canary.semantic_projector_wr2c5 import project


ROOT = Path(__file__).resolve().parents[2]
SOURCE = Path(__file__).resolve()
EXTRACTOR = ROOT / "experiments/world_runtime_writer_canary/semantic_extractor_wr2c5.py"
PROJECTOR = ROOT / "experiments/world_runtime_writer_canary/semantic_projector_wr2c5.py"
VALIDATOR = ROOT / "experiments/world_runtime_writer_canary/delta_shadow_wr2c5.py"
FIXTURE = ROOT / "experiments/world_runtime_writer_canary/fixtures/wr2c5_semantic_development_v5.json"
EXTERNAL_RUNNER = ROOT / "experiments/world_runtime_writer_canary/external_runner_wr2c5.py"
DEFAULT_OUTPUT = ROOT / ".world_runtime_wr2c5_semantic_canary_runtime"
...
def build(output_dir: Path = DEFAULT_OUTPUT) -> dict[str, Any]:
    ...
    manifest = {
        "schema_version": "world-runtime-semantic-canary-wr2c5-manifest-v1",
        "experiment_id": "world-runtime-semantic-judgment-wr2c5-development-v5",
        "partition_role": "visible_development_v5_not_holdout",
    ...
    ledger = {
        "schema_version": "world-runtime-semantic-canary-wr2c5-ledger-v1",
        "samples": {
            sample["sample_id"]: {
...
def _assert_integrity(output_dir: Path) -> dict[str, Any]:
    ...
    for field, expected in checks.items():
        if manifest.get(field) != expected:
            raise RuntimeError(f"wr2c5_frozen_drift:{field}")
    return manifest
...
def audit(output_dir: Path = DEFAULT_OUTPUT) -> dict[str, Any]:
    ...
    result = {
        "schema_version": "world-runtime-semantic-canary-wr2c5-preflight-v1",
...
def evaluate(output_dir: Path = DEFAULT_OUTPUT) -> dict[str, Any]:
    ...
    result = {
        "schema_version": "world-runtime-semantic-canary-wr2c5-evaluation-v1",
        "partition_role": "visible_development_v5_not_holdout",
        "sample_count": manifest["sample_count"],
        ...
        "state_mutations": 0,
        "commits": 0,
        "decision": "development_v5_passed_new_unseen_holdout_required" if passed else "hold_semantic_development_v5_failed",
        "items": items,
    }
