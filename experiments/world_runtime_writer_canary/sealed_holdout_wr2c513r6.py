"""WR2-C5.1.3-R6 sealed unseen holdout runner (fixed component binding).
...
EXTRACTOR = ROOT / "experiments/world_runtime_writer_canary/semantic_extractor_wr2c513r4.py"
PROJECTOR = ROOT / "experiments/world_runtime_writer_canary/semantic_projector_wr2c513r4.py"
VALIDATOR = ROOT / "experiments/world_runtime_writer_canary/delta_shadow_wr2c5.py"
RUNTIME = ROOT / ".world_runtime_wr2c513r6_sealed_holdout_runtime"
...
def _assert_component_binding() -> None:
    if EXTRACTOR.name != "semantic_extractor_wr2c513r4.py":
        raise RuntimeError(f"holdout runner bound to wrong extractor: {EXTRACTOR.name}")
    if PROJECTOR.name != "semantic_projector_wr2c513r4.py":
...
def seal(output_dir: Path = RUNTIME) -> dict[str, Any]:
    ...
    lock = {
        "schema_version": "world-runtime-sealed-holdout-wr2c513r6-lock-v1",
        "experiment_id": "world-runtime-semantic-judgment-wr2c513r6-sealed-unseen-holdout",
...
def build(output_dir: Path = RUNTIME) -> dict[str, Any]:
    ...
    for ordinal, case in enumerate(cases, 1):
        ...
        sample_id = f"WR2C513R6H-{ordinal:02d}"
        samples.append({
            "sample_id": sample_id,
    ...
    manifest = {
        "schema_version": "world-runtime-sealed-holdout-wr2c513r6-manifest-v1",
        "experiment_id": "world-runtime-semantic-judgment-wr2c513r6-sealed-unseen-holdout",
        "partition_role": "sealed_unseen_holdout",
        "holdout_sha256": _sha256_file(HOLDOUT_PATH),
    ...
    _write_json(manifest_path, manifest)
    ledger = {
        "schema_version": "world-runtime-sealed-holdout-wr2c513r6-ledger-v1",
        "samples": {
            sample["sample_id"]: {
...
def audit(output_dir: Path = RUNTIME) -> dict[str, Any]:
    ...
    return {
        "schema_version": "world-runtime-sealed-holdout-wr2c513r6-preflight-v1",
        "ready": not issues,
        "issues": issues,
...
def preflight(
    ...
    expected = {
        "schema_version": "world-runtime-semantic-canary-wr2c513r6-sealed-holdout-external-authorization-v1",
        "authorized": True,
    ...
    return {
        "schema_version": "world-runtime-sealed-holdout-wr2c513r6-external-preflight-v1",
        "ready": not issues,
        "issues": issues,
...
def evaluate(output_dir: Path = RUNTIME) -> dict[str, Any]:
    ...
    result = {
        "schema_version": "world-runtime-sealed-holdout-wr2c513r6-evaluation-v1",
        "partition_role": "sealed_unseen_holdout",
        "sample_count": manifest["sample_count"],
...
def run_once(
    ...
    for sample in manifest["samples"]:
        ...
        try:
            response = client.chat_completion(
                ...
                prompt_name="world_runtime_semantic_judgment_wr2c513r6_holdout_v1",
    ...
    return {
        "schema_version": "world-runtime-sealed-holdout-wr2c513r6-external-result-v1",
