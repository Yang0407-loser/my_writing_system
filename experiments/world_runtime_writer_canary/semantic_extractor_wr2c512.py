"""WR2-C5.1.2 semantic judgment contract and parser.

Same judgment+evidence contract as WR2-C5.1.1 (no prompt change; projector adds
multi-clock fallback and composite object split).
...
from experiments.world_runtime_writer_canary.semantic_projector_wr2c512 import (
    build_delta,
    project,
)


SEMANTIC_EXTRACTOR_VERSION = "world-runtime-semantic-extractor-wr2c512-v1"
MAX_JUDGMENTS = 32
...
class SemanticExtractionArtifactC512(FrozenModel):
...
def parse_semantic_response(
    *,
...
) -> SemanticExtractionArtifactC512:
    ...
    try:
        ...
        raise ValueError("wr2c512_invalid_judgment_response") from exc
    ...
    if delta.output_hash != output_hash:
        raise RuntimeError("wr2c512_output_hash_mismatch")
    return SemanticExtractionArtifactC512(
        sample_id=sample_id,
