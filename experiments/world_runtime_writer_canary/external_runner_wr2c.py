"""One-shot external executor for the frozen WR2-C semantic canary."""
...
from experiments.world_runtime_writer_canary import semantic_canary_wr2c as wr2c


SOURCE = Path(__file__).resolve()
DEFAULT_AUTHORIZATION = wr2c.DEFAULT_OUTPUT / "private/external-execution-authorization.json"
...
def preflight(
    output_dir: Path = wr2c.DEFAULT_OUTPUT,
    authorization_path: Path = DEFAULT_AUTHORIZATION,
) -> dict[str, Any]:
    audit = wr2c.audit(output_dir)
    ...
    return {
        "schema_version": "world-runtime-semantic-canary-wr2c-external-preflight-v1",
        "ready": not issues,
        "issues": issues,
        "provider_host": manifest["provider_host"], "model": manifest["model"],
        "sample_count": manifest["sample_count"], "pending": audit["pending"],
        "attempt_count_total": audit["attempt_count_total"], "transport_retries": 0,
...
def run_once(
    ...
    if not check["ready"]:
        raise RuntimeError("wr2c_external_preflight_failed:" + "|".join(check["issues"]))
    ...
    for sample in manifest["samples"]:
        ...
        if entry["status"] != "pending" or entry["attempt_count"] != 0:
            raise RuntimeError(f"wr2c_refusing_nonpristine_sample:{sample['sample_id']}")
        entry.update(status="started", attempt_count=1)
        _write(ledger_path, ledger)
        metadata: dict[str, Any] = {}
        try:
            response = client.chat_completion(
                sample["messages"], temperature=sample["provider"]["temperature"],
                max_tokens=sample["provider"]["max_tokens"], max_retries=0,
                json_mode=sample["provider"]["json_mode"],
                prompt_name="world_runtime_semantic_extractor_wr2c_v1",
            ...
            raise
        _write(ledger_path, ledger)
    evaluation = wr2c.evaluate(output_dir)
    return {
        "schema_version": "world-runtime-semantic-canary-wr2c-external-result-v1",
        "command_executed_exactly_once": True,
        "succeeded": completed, "failed": 0,
...
def main() -> None:
    ...
    parser.add_argument("--output", type=Path, default=wr2c.DEFAULT_OUTPUT)
