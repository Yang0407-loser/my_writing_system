"""WR2-C4 deterministic projector.

Same boundary as WR2-C3 (model judges, projector owns canonical fields) plus
two surgical fixes:
...
PROJECTOR_VERSION = "world-runtime-projector-wr2c4-v1"
...
def _resolve_knowledge_mechanism(evidence: str) -> str:
    ...
    if _has_any(evidence, ("文件", "文档", "全文", "定稿")) and "群" in evidence:
    ...
    *,
    ...
    for sequence, event in enumerate(events, 1):
        ...
        for evidence_index, (evidence, span) in enumerate(event.evidence, 1):
            evidence_id = f"ev:wr2c4:{sample_id.lower()}:{event.judgment_index + 1}:{evidence_index}"
        ...
        changes.append(ProposedChangeV2(
            change_id=f"change:wr2c4:{sample_id.lower()}:{sequence}",
            sequence=sequence,
    ...
    return ProposedTypedDeltaV2(
        delta_id=f"delta:wr2c4:{sample_id.lower()}",
        sample_id=sample_id,
        scene_id=scene_id,
