"""WR2-C5.1.3-R5 deterministic projector.
...
PROJECTOR_VERSION = "world-runtime-projector-wr2c513r5-v1"
...
def _parse_clock(text: str) -> str | None:
    for pattern in _CLOCK_PATTERNS:
        ...
        if ":" in matched or "：" in matched:
            ...
            if hour is None:
                continue
            minute = _cn_number(groups[2]) or 0
...
def _is_bare_schedule_reference(text: str, match) -> bool:
    ...
    return any(verb in tail for verb in _SCHEDULE_VERBS)


def _evidence_is_bare_schedule_reference(excerpt: str) -> bool:
    return re.search(
        r"([零一二三四五六七八九十两\d]{1,3})点(开门|关门|打烊|出炉|上班|下班|开门营业|开始营业)",
        excerpt,
    ) is not None
    ...
    *,
    ...
    for sequence, event in enumerate(events, 1):
        ...
        for evidence_index, (evidence, span) in enumerate(event.evidence, 1):
            evidence_id = f"ev:wr2c513r5:{sample_id.lower()}:{sequence}:{evidence_index}"
        ...
        changes.append(ProposedChangeV5(
            change_id=f"change:wr2c513r5:{sample_id.lower()}:{sequence}",
            sequence=sequence,
            change_type=event.change_type,
    ...
    return ProposedTypedDeltaV5(
        delta_id=f"delta:wr2c513r5:{sample_id.lower()}",
        sample_id=sample_id,
        scene_id=scene_id,
