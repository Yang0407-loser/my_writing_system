"""WR2-C zero-call semantic extractor canary builder and evaluator."""
...
EXTERNAL_RUNNER = ROOT / "experiments/world_runtime_writer_canary/external_runner_wr2c.py"
DEFAULT_OUTPUT = ROOT / ".world_runtime_wr2c_semantic_canary_runtime"
SELECTED_CASE_IDS = (
    "ADV-C-S01", "ADV-S02", "ADV-K01", "ADV-C-O01", "ADV-U01",
    "ADV-O01", "ADV-O02", "ADV-R01", "ADV-E01", "ADV-E03",
    "ADV-P01", "ADV-D01", "ADV-D02", "ADV-T01", "ADV-L01",
    "ADV-S01", "ADV-S03", "ADV-K02", "ADV-K04", "ADV-R02",
)
...
def _signature(change: dict[str, Any]) -> tuple[str, str, str, str, str]:
    return (
        change["change_type"], change["subject"], change["predicate"],
        json.dumps(change.get("after_value"), ensure_ascii=False, sort_keys=True), change["mechanism"],
    )


def build(output_dir: Path = DEFAULT_OUTPUT) -> dict[str, Any]:
    if (output_dir / "attempt-ledger.json").exists():
        raise FileExistsError("WR2-C attempt ledger exists; refusing rebuild")
    if not EXTERNAL_RUNNER.exists():
        raise FileNotFoundError("WR2-C external runner missing")
    fixture = _read(FIXTURE)
    by_id = {item["case_id"]: item for item in fixture["cases"]}
    if set(SELECTED_CASE_IDS) - set(by_id):
        raise ValueError("WR2-C selected case missing")
    ...
    expected_changes = [change for sample in samples for change in sample["expected_changes"]]
    change_types = {change["change_type"] for change in expected_changes}
    if len(samples) != GATES["sample_count"] or len(change_types) != 13:
        raise ValueError("WR2-C sample or ontology coverage mismatch")
    manifest = {
        "schema_version": "world-runtime-semantic-canary-wr2c-manifest-v1",
        "experiment_id": "world-runtime-semantic-extractor-wr2c-development-v1",
        "partition_role": "visible_development_not_holdout",
        "fixture_sha256": _sha256(FIXTURE),
        "builder_source_sha256": _sha256(SOURCE),
        "semantic_extractor_source_sha256": _sha256(ROOT / "experiments/world_runtime_writer_canary/semantic_extractor_wr2c.py"),
        "validator_source_sha256": _sha256(ROOT / "experiments/world_runtime_writer_canary/delta_shadow_wr2b.py"),
    ...
    ledger = {
        "schema_version": "world-runtime-semantic-canary-wr2c-ledger-v1",
        "samples": {
            sample["sample_id"]: {
                "request_hash": sample["request_hash"], "status": "pending", "attempt_count": 0,
            }
...
def _assert_integrity(output_dir: Path) -> dict[str, Any]:
    ...
    for field, expected in checks.items():
        if manifest.get(field) != expected:
            raise RuntimeError(f"wr2c_frozen_drift:{field}")
    return manifest
...
def audit(output_dir: Path = DEFAULT_OUTPUT) -> dict[str, Any]:
    ...
    outputs = list((output_dir / "private/outputs").glob("*.json")) if (output_dir / "private/outputs").exists() else []
    ...
    result = {
        "schema_version": "world-runtime-semantic-canary-wr2c-preflight-v1",
        "status": "ready_zero_call_external_execution_not_authorized" if not issues else "blocked",
        "issues": issues,
        "provider_host": manifest["provider_host"], "model": manifest["model"],
        "sample_count": manifest["sample_count"], "expected_change_count": manifest["expected_change_count"],
        "expected_empty_count": manifest["expected_empty_count"], "all_13_types_covered": manifest["all_13_types_covered"],
        "pending": pending, "attempt_count_total": attempts, "output_files": len(outputs),
        "estimated_input_tokens_mean": round(statistics.mean(token_estimates), 2),
        "estimated_input_tokens_max": max(token_estimates),
        "transport_retries": 0, "provider_calls_executed": 0,
        "production_default": settings.WRITER_WORLD_RUNTIME_MODE,
        "sealed_holdout_used": False, "state_commit_authorized": False,
...
def evaluate(output_dir: Path = DEFAULT_OUTPUT) -> dict[str, Any]:
    ...
    for sample in manifest["samples"]:
        ...
        if entry["status"] != "succeeded" or entry["attempt_count"] != 1:
            raise RuntimeError("WR2-C requires exactly one succeeded attempt per sample")
        ...
        if hashlib.sha256(response_text.encode("utf-8")).hexdigest() != entry["output_sha256"]:
            raise RuntimeError("WR2-C response hash mismatch")
        try:
            artifact = parse_semantic_response(
                text=sample["text"], response_text=response_text, sample_id=sample["sample_id"],
                scene_id=sample["scene_id"], state_variant=sample["state_variant"],
        ...
        totals["invalid_expected"] += sum(item["expected_validation"] == "invalid" for item in sample["expected_changes"])
        ...
        if not sample["expected_changes"]:
            totals["empty_expected"] += 1
            totals["empty_correct"] += int(not artifact.delta.changes)
        totals["evidence"] += len(artifact.delta.evidence)
        totals["evidence_ok"] += sum(
            sample["text"][evidence.start:evidence.end] == evidence.excerpt for evidence in artifact.delta.evidence
        )
        ...
        items.append({
            ...
            "raw_event_count": artifact.raw_event_count, "projected_event_count": artifact.projected_event_count,
            "dropped_event_count": len(artifact.dropped_events), "expected_change_count": len(sample["expected_changes"]),
            "matched_change_count": matched, "validation_correct_count": validation_correct,
            "unsupported_accepted_count": unsupported, "would_commit": validation.would_commit,
    ...
    gates = {
        ...
        "commit_forbidden": all(not item.get("would_commit", False) and not item.get("state_mutated", False) for item in items),
    }
    passed = all(gates.values())
    result = {
        "schema_version": "world-runtime-semantic-canary-wr2c-evaluation-v1",
        "partition_role": "visible_development_not_holdout",
        "sample_count": manifest["sample_count"], "expected_change_count": totals["expected"],
        "extracted_change_count": totals["extracted"], "matched_change_count": totals["matched"],
        "semantic_precision": precision, "semantic_recall": recall,
        "matched_validation_accuracy": validation_accuracy, "invalid_transition_recall": invalid_recall,
        "empty_delta_correctness": empty_correctness, "parser_failure_count": totals["parser_failures"],
        "unsupported_accepted_change_count": totals["unsupported_accepted"],
        "evidence_traceability": evidence_rate, "dropped_event_count": totals["dropped"],
        "mean_elapsed_ms": mean_elapsed, "gates": gates, "development_gate_passed": passed,
        "production_promotion_eligible": False, "sealed_holdout_authorized": False,
        "state_mutations": 0, "commits": 0,
        "decision": "semantic_development_signal_passed_new_holdout_required" if passed else "hold_semantic_development_failed",
        "items": items,
    }
...
def main() -> None:
    ...
    function = {"build": build, "audit": audit, "evaluate": evaluate}[args.command]
