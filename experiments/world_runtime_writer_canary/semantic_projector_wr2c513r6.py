"""WR2-C5.1.3-R6 deterministic projector.

Same boundary as WR2-C5.1.3-R3, with one surgical fix: clock references that
point to a past or future day (e.g. "昨晚十点", "明天六点") are no longer
treated as current-scene time progression.  The guard applies to both the
deterministic clock fallback and clock judgments whose evidence contains the
shifted-day marker.

The R3 actor fix, clock time ordering, repeat subsumption, open fallback,
multi-clock fallback and composite split are unchanged.
...
PROJECTOR_VERSION = "world-runtime-projector-wr2c513r6-v1"
...
def _cn_number(token: str | None) -> int | None:
    ...
    if "十" in token:
        ...
        return tens * 10 + ones
    if token.startswith("零") and len(token) > 1:
        rest = token[1:]
        value = _cn_number(rest)
...
_CLOCK_PATTERNS = (
    ...
    re.compile(r"([零一二三四五六七八九十两\d]{1,3})点差(一刻|[零一二三四五六七八九十两\d]{1,3}分)"),
    re.compile(r"([零一二三四五六七八九十两\d]{1,3})点过(一刻|[零一二三四五六七八九十两\d]{1,3}分)"),
    re.compile(r"([零一二三四五六七八九十两\d]{1,3})点一刻"),
    re.compile(r"([零一二三四五六七八九十两\d]{1,3})点半"),
    re.compile(r"([零一二三四五六七八九十两\d]{1,3})点([零一二三四五六七八九十两\d]{1,3})?(?:分)?"),
)
...
def _parse_clock(text: str) -> str | None:
    for pattern in _CLOCK_PATTERNS:
        ...
        if ":" in matched or "：" in matched:
            ...
            if hour is None:
                continue
            minutes = 15 if "一刻" in matched else (_cn_number(groups[1].rstrip("分")) or 0)
            hour, minute = divmod((hour * 60 - minutes) % 1440, 60)
        elif "过" in matched:
            hour = _cn_number(groups[0])
            if hour is None:
                continue
            minute = 15 if "一刻" in matched else (_cn_number(groups[1].rstrip("分")) or 0)
        elif "一刻" in matched:
            hour = _cn_number(groups[0])
            if hour is None:
                continue
            ...
            if hour is None:
                continue
            minute = _cn_number(groups[1]) or 0
...
def _is_bare_schedule_reference(text: str, match) -> bool:
    """Return True when a bare hour is a schedule/plan statement, not scene time."""
    token = match.group(0)
    if any(marker in token for marker in ("分", "整", "半", "刻", ":", "：")):
        return False
    tail = text[match.end():match.end() + 8]
    return any(verb in tail for verb in _SCHEDULE_VERBS) or tail.startswith(
        ("前", "后", "左右", "之前", "之后")
    )


def _evidence_is_bare_schedule_reference(excerpt: str) -> bool:
    return re.search(
        r"([零一二三四五六七八九十两\d]{1,3})点(开门|关门|打烊|出炉|上班|下班|开门营业|开始营业|前|后|左右|之前|之后)",
        excerpt,
    ) is not None


_SHIFTED_TIME_MARKERS = (
    "昨晚", "昨天", "前天", "前晚",
    "明晚", "明天", "后天", "后晚",
    "上周", "上星期", "下周", "下星期",
)
...
def _is_shifted_time_reference(context: str) -> bool:
    """Return True when the surrounding text names a past/future day."""
    return any(marker in context for marker in _SHIFTED_TIME_MARKERS)
...
def _fallback_clock_events(
    *,
    ...
    candidates = [
        ...
        for pattern_index, pattern in enumerate(_CLOCK_PATTERNS)
        for match in pattern.finditer(text)
    ]
    candidates.sort(key=lambda item: (item[0], item[1]))
    last_end = -1
    for _start, _pattern_index, match in candidates:
        if match.start() < last_end:
            continue
        last_end = match.end()
        if any(start <= match.start() < end for start, end in quoted):
            continue
        if _is_shifted_time_reference(text[max(0, match.start() - 6):match.start()]):
            continue
        if _is_bare_schedule_reference(text, match):
            continue
...
_OPEN_FALLBACK_PATTERNS = (
    re.compile(r"(拉开|推开|拉起|打开)(卷帘门|临街门|店门|大门)"),
    re.compile(r"(卷帘门|临街门|店门|大门)(拉开|推开|拉起|打开)"),
    re.compile(r"开门营业|开始营业|正式营业|拉开卷帘门"),
)


def _fallback_operation_event(
    *,
    text: str,
    judgments: list[dict[str, Any]],
) -> ProjectedEvent | None:
    """Synthesize a storefront open change from explicit door-opening tokens."""

    quoted = _quoted_spans(text)
    for pattern in _OPEN_FALLBACK_PATTERNS:
        for match in pattern.finditer(text):
            if any(start <= match.start() < end for start, end in quoted):
                continue
            evidence = RawEvidence(excerpt=match.group(0), occurrence=1)
            return ProjectedEvent(
                ...
                after_value="open",
                actor="narrator",
                mechanism="explicit_open_close",
                evidence=[(evidence, (match.start(), match.end()))],
...
def _sort_key(event: ProjectedEvent) -> tuple[int, str, int]:
    clock_time = event.after_value if event.change_type == "clock_state" else ""
    return (_CHAIN_RANK.get(event.change_type, 9), clock_time, event.judgment_index)
...
def _resolve_object(
    ...
    actor_present = evidence_actor is not None or unique_actor is not None
...
def _resolve_judgment(
    ...
    if change_type == "clock_state":
        if _is_shifted_time_reference(evidence_text) or _evidence_is_bare_schedule_reference(evidence_text):
            return None
    ...
    *,
    ...
    if any(event.change_type == "storefront_public_sale" for event in events):
        ...
        for event in events:
            ...
            kept.append(event)
        events = kept
    if any(event.change_type == "repeated_completed_event" for event in events):
        kept = [
            event
            for event in events
            if event.change_type != "publication_state"
            or not any(
                other.change_type == "repeated_completed_event"
                for other in events
            )
        ]
        dropped.extend(
            DroppedEvent(
                index=event.judgment_index,
                reason="publication_subsumed_by_repeat",
                change_type=event.change_type,
            )
            for event in events
            if event.change_type == "publication_state"
    ...
    if fallback:
        ...
        events.sort(key=_sort_key)
    if not any(event.change_type == "storefront_operation_state" for event in events):
        operation = _fallback_operation_event(text=text, judgments=judgments)
    ...
    *,
    ...
    for sequence, event in enumerate(events, 1):
        ...
        for evidence_index, (evidence, span) in enumerate(event.evidence, 1):
            evidence_id = f"ev:wr2c513r6:{sample_id.lower()}:{sequence}:{evidence_index}"
        ...
        changes.append(ProposedChangeV5(
            change_id=f"change:wr2c513r6:{sample_id.lower()}:{sequence}",
            sequence=sequence,
            change_type=event.change_type,
    ...
    return ProposedTypedDeltaV5(
        delta_id=f"delta:wr2c513r6:{sample_id.lower()}",
        sample_id=sample_id,
        scene_id=scene_id,
