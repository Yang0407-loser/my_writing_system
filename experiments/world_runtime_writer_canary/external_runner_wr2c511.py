"""One-shot external executor for the frozen WR2-C4 Development-v4 canary."""
...
from experiments.world_runtime_writer_canary import semantic_canary_wr2c511 as wr2c511


SOURCE = Path(__file__).resolve()
DEFAULT_AUTHORIZATION = wr2c511.DEFAULT_OUTPUT / "private/external-execution-authorization.json"
...
def preflight(
    output_dir: Path = wr2c511.DEFAULT_OUTPUT,
    authorization_path: Path = DEFAULT_AUTHORIZATION,
) -> dict[str, Any]:
    audit = wr2c511.audit(output_dir)
    ...
    expected = {
        "schema_version": "world-runtime-semantic-canary-wr2c511-external-authorization-v1",
        "authorized": True,
    ...
    return {
        "schema_version": "world-runtime-semantic-canary-wr2c511-external-preflight-v1",
        "ready": not issues,
        "issues": issues,
...
def run_once(
    ...
    if not check["ready"]:
        raise RuntimeError("wr2c511_external_preflight_failed:" + "|".join(check["issues"]))
    ...
    for sample in manifest["samples"]:
        ...
        if entry["status"] != "pending" or entry["attempt_count"] != 0:
            raise RuntimeError(f"wr2c511_refusing_nonpristine_sample:{sample['sample_id']}")
        ...
        try:
            response = client.chat_completion(
                ...
                prompt_name="world_runtime_semantic_judgment_wr2c511_v1",
            ...
            raise
        _write(ledger_path, ledger)
    evaluation = wr2c511.evaluate(output_dir)
    return {
        "schema_version": "world-runtime-semantic-canary-wr2c511-external-result-v1",
...
def main() -> None:
    ...
    parser.add_argument("--output", type=Path, default=wr2c511.DEFAULT_OUTPUT)
