"""WR2-C5.1.3 sealed unseen holdout runner.
...
RUNTIME = ROOT / ".world_runtime_wr2c513_sealed_holdout_runtime"
...
def seal(output_dir: Path = RUNTIME) -> dict[str, Any]:
    ...
    lock = {
        "schema_version": "world-runtime-sealed-holdout-wr2c513-lock-v1",
        "experiment_id": "world-runtime-semantic-judgment-wr2c513-sealed-unseen-holdout",
...
def build(output_dir: Path = RUNTIME) -> dict[str, Any]:
    ...
    for ordinal, case in enumerate(cases, 1):
        ...
        sample_id = f"WR2C513H-{ordinal:02d}"
        samples.append({
            "sample_id": sample_id,
    ...
    manifest = {
        "schema_version": "world-runtime-sealed-holdout-wr2c513-manifest-v1",
        "experiment_id": "world-runtime-semantic-judgment-wr2c513-sealed-unseen-holdout",
        "partition_role": "sealed_unseen_holdout",
        "holdout_sha256": _sha256_file(HOLDOUT_PATH),
    ...
    _write_json(manifest_path, manifest)
    ledger = {
        "schema_version": "world-runtime-sealed-holdout-wr2c513-ledger-v1",
        "samples": {
            sample["sample_id"]: {
...
def audit(output_dir: Path = RUNTIME) -> dict[str, Any]:
    ...
    return {
        "schema_version": "world-runtime-sealed-holdout-wr2c513-preflight-v1",
        "ready": not issues,
        "issues": issues,
...
def preflight(
    ...
    return {
        "schema_version": "world-runtime-sealed-holdout-wr2c513-external-preflight-v1",
        "ready": not issues,
        "issues": issues,
...
def evaluate(output_dir: Path = RUNTIME) -> dict[str, Any]:
    ...
    result = {
        "schema_version": "world-runtime-sealed-holdout-wr2c513-evaluation-v1",
        "partition_role": "sealed_unseen_holdout",
        "sample_count": manifest["sample_count"],
...
def run_once(
    ...
    for sample in manifest["samples"]:
        ...
        try:
            response = client.chat_completion(
                ...
                prompt_name="world_runtime_semantic_judgment_wr2c513_holdout_v1",
    ...
    return {
        "schema_version": "world-runtime-sealed-holdout-wr2c513-external-result-v1",
