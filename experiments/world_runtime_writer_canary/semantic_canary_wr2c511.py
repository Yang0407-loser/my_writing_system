"""WR2-C4 Development-v4 semantic judgment canary builder and evaluator."""
...
FIXTURE = ROOT / "experiments/world_runtime_writer_canary/fixtures/wr2c511_semantic_development_v511.json"
EXTERNAL_RUNNER = ROOT / "experiments/world_runtime_writer_canary/external_runner_wr2c511.py"
DEFAULT_OUTPUT = ROOT / ".world_runtime_wr2c511_semantic_canary_runtime"
...
def _projector_roundtrip(case: dict[str, Any]) -> None:
    """Gold judgments must project exactly onto the frozen expected changes."""
    ...
    if actual != expected:
        raise ValueError(
            f"WR2-C4 projector roundtrip mismatch in {case['case_id']}: "
...
def build(output_dir: Path = DEFAULT_OUTPUT) -> dict[str, Any]:
    if (output_dir / "attempt-ledger.json").exists():
        raise FileExistsError("WR2-C4 attempt ledger exists; refusing rebuild")
    if not EXTERNAL_RUNNER.exists():
        raise FileNotFoundError("WR2-C4 external runner missing")
    ...
    for ordinal, case in enumerate(cases, 1):
        ...
        sample_id = f"WR2C511-{ordinal:02d}"
        samples.append({
            "sample_id": sample_id,
    ...
    if len(samples) != GATES["sample_count"] or len(change_types) != 14:
        raise ValueError("WR2-C4 sample or ontology coverage mismatch")
    manifest = {
        "schema_version": "world-runtime-semantic-canary-wr2c511-manifest-v1",
        "experiment_id": "world-runtime-semantic-judgment-wr2c511-development-v511",
        "partition_role": "visible_development_v511_not_holdout",
        "fixture_sha256": _sha256(FIXTURE),
        "builder_source_sha256": _sha256(SOURCE),
        "semantic_extractor_source_sha256": _sha256(EXTRACTOR),
        "projector_source_sha256": _sha256(PROJECTOR),
    ...
    ledger = {
        "schema_version": "world-runtime-semantic-canary-wr2c511-ledger-v1",
        "samples": {
            sample["sample_id"]: {
...
def _assert_integrity(output_dir: Path) -> dict[str, Any]:
    ...
    for field, expected in checks.items():
        if manifest.get(field) != expected:
            raise RuntimeError(f"wr2c511_frozen_drift:{field}")
    return manifest
...
def audit(output_dir: Path = DEFAULT_OUTPUT) -> dict[str, Any]:
    ...
    result = {
        "schema_version": "world-runtime-semantic-canary-wr2c511-preflight-v1",
        ...
        "all_14_types_covered": manifest["all_14_types_covered"],
        "pending": pending,
...
def evaluate(output_dir: Path = DEFAULT_OUTPUT) -> dict[str, Any]:
    ...
    for sample in manifest["samples"]:
        ...
        if entry["status"] != "succeeded" or entry["attempt_count"] != 1:
            raise RuntimeError("WR2-C4 requires exactly one succeeded attempt per sample")
        ...
        if hashlib.sha256(response_text.encode("utf-8")).hexdigest() != entry["output_sha256"]:
            raise RuntimeError("WR2-C4 response hash mismatch")
    ...
    result = {
        "schema_version": "world-runtime-semantic-canary-wr2c511-evaluation-v1",
        "partition_role": "visible_development_v511_not_holdout",
        "sample_count": manifest["sample_count"],
        ...
        "state_mutations": 0,
        "commits": 0,
        "decision": "development_v511_passed_new_unseen_holdout_required" if passed else "hold_semantic_development_v511_failed",
        "items": items,
    }
