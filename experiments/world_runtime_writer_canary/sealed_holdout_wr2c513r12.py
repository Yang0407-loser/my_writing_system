"""WR2-C5.1.3-R12 sealed unseen holdout runner (fixed component binding).
...
from experiments.world_runtime_writer_canary.semantic_canary_wr2c511 import GATES
from experiments.world_runtime_writer_canary.semantic_extractor_wr2c513r10 import (
    RawJudgmentResponse,
    build_messages,
    parse_semantic_response,
)
from experiments.world_runtime_writer_canary.semantic_projector_wr2c513r6 import project


ROOT = Path(__file__).resolve().parents[2]
SOURCE = Path(__file__).resolve()
EXTRACTOR = ROOT / "experiments/world_runtime_writer_canary/semantic_extractor_wr2c513r10.py"
...
RUNTIME = ROOT / ".world_runtime_wr2c513r12_sealed_holdout_runtime"
...
DEVELOPMENT_FIXTURES = (
    ...
    ROOT / "experiments/world_runtime_writer_canary/fixtures/wr2c5_semantic_development_v5.json",
    ROOT / "experiments/world_runtime_writer_canary/fixtures/wr2c51_semantic_development_v51.json",
    ROOT / "experiments/world_runtime_writer_canary/fixtures/wr2c511_semantic_development_v511.json",
)
WR2B_SEALED_HOLDOUT = ROOT / ".world_runtime_wr2b_sealed_holdout_runtime/private/sealed-holdout-v1.json"
WR2C4_SEALED_HOLDOUT = ROOT / ".world_runtime_wr2c4_sealed_holdout_runtime/private/sealed-holdout-v1.json"
WR2C511_SEALED_HOLDOUT = ROOT / ".world_runtime_wr2c511_sealed_holdout_runtime/private/sealed-holdout-v1.json"
WR2C512_SEALED_HOLDOUT = ROOT / ".world_runtime_wr2c512_sealed_holdout_runtime/private/sealed-holdout-v1.json"
FAILED_513_HOLDOUT = ROOT / ".world_runtime_wr2c513_sealed_holdout_runtime/private/sealed-holdout-v1.json"
FAILED_513R2_HOLDOUT = ROOT / ".world_runtime_wr2c513r2_sealed_holdout_runtime/private/sealed-holdout-v1.json"
FAILED_513R3_HOLDOUT = ROOT / ".world_runtime_wr2c513r3_sealed_holdout_runtime/private/sealed-holdout-v1.json"
WR2C513R4_SEALED_HOLDOUT = ROOT / ".world_runtime_wr2c513r4_sealed_holdout_runtime/private/sealed-holdout-v1.json"
WR2C513R5_SEALED_HOLDOUT = ROOT / ".world_runtime_wr2c513r5_sealed_holdout_runtime/private/sealed-holdout-v1.json"
WR2C513R6_SEALED_HOLDOUT = ROOT / ".world_runtime_wr2c513r6_sealed_holdout_runtime/private/sealed-holdout-v1.json"
WR2C513R7_SEALED_HOLDOUT = ROOT / ".world_runtime_wr2c513r7_sealed_holdout_runtime/private/sealed-holdout-v1.json"
WR2C513R8_SEALED_HOLDOUT = ROOT / ".world_runtime_wr2c513r8_sealed_holdout_runtime/private/sealed-holdout-v1.json"
WR2C513R9_SEALED_HOLDOUT = ROOT / ".world_runtime_wr2c513r9_sealed_holdout_runtime/private/sealed-holdout-v1.json"
WR2C513R10_SEALED_HOLDOUT = ROOT / ".world_runtime_wr2c513r10_sealed_holdout_runtime/private/sealed-holdout-v1.json"
WR2C513R11_SEALED_HOLDOUT = ROOT / ".world_runtime_wr2c513r11_sealed_holdout_runtime/private/sealed-holdout-v1.json"
PRIOR_SEALED_HOLDOUTS = (
    WR2B_SEALED_HOLDOUT,
    WR2C4_SEALED_HOLDOUT,
    WR2C511_SEALED_HOLDOUT,
    WR2C512_SEALED_HOLDOUT,
    FAILED_513_HOLDOUT,
    FAILED_513R2_HOLDOUT,
    FAILED_513R3_HOLDOUT,
    WR2C513R4_SEALED_HOLDOUT,
    WR2C513R5_SEALED_HOLDOUT,
    WR2C513R6_SEALED_HOLDOUT,
    WR2C513R7_SEALED_HOLDOUT,
    WR2C513R8_SEALED_HOLDOUT,
    WR2C513R9_SEALED_HOLDOUT,
    WR2C513R10_SEALED_HOLDOUT,
    WR2C513R11_SEALED_HOLDOUT,
)
...
def _has_cross_paragraph_multi_evidence(case: dict[str, Any]) -> bool:
    """Return True when one judgment splits evidence across a paragraph break."""
    text = case.get("text", "")
    for judgment in case.get("judgments", []):
        evidence = judgment.get("evidence") or []
        if not judgment.get("occurred") or len(evidence) < 2:
            continue
        spans = []
        ...
        if len(spans) == 2:
            low = min(spans[0][1], spans[1][1])
            high = max(spans[0][0], spans[1][0])
            if "\n\n" in text[low:high]:
                return True
    return False
...
def _development_text_hashes() -> set[str]:
    ...
    for holdout_path in PRIOR_SEALED_HOLDOUTS:
...
def _prevalidate_gold(case: dict[str, Any]) -> None:
    """Assert that gold expected_validation matches the frozen Validator."""

    if not case.get("judgments") and not case.get("changes"):
        return
    _, states, _ = wr1r._artifacts()
    state = states[case["state_variant"]]
    artifact = parse_semantic_response(
        text=case["text"],
        response_text=json.dumps({"judgments": case["judgments"]}, ensure_ascii=False),
        sample_id="GOLD-PREVALIDATE",
        scene_id=case["scene_id"],
        state_variant=case["state_variant"],
        base_revision=state.revision,
    )
    validation = validate_delta_v6(artifact.delta)
    outcomes = {
        item.change_id: item.outcome
        for item in validation.items
    }
    actual_by_signature: dict[tuple[str, str, str, str, str], str | None] = {}
    for change in artifact.delta.changes:
        signature = (
            change.change_type,
            change.subject,
            change.predicate,
            json.dumps(change.after_value, ensure_ascii=False, sort_keys=True),
            change.mechanism,
        )
        actual_by_signature[signature] = outcomes.get(change.change_id)
    for expected in case["changes"]:
        signature = _signature(expected)
        actual = actual_by_signature.get(signature)
        if actual is None:
            raise ValueError(
                f"holdout gold change not produced by projector: {case['case_id']} "
                f"{expected['change_type']}"
            )
        if actual != expected["expected_validation"]:
            raise ValueError(
                f"holdout gold validation mismatch in {case['case_id']}: "
                f"{expected['change_type']} expected={expected['expected_validation']} "
                f"validator={actual}"
            )
...
def validate_holdout(holdout: dict[str, Any]) -> dict[str, Any]:
    ...
    has_operation = any(c["change_type"] == "storefront_operation_state" for c in expected)
    if len(change_types) != 14:
        raise ValueError(f"holdout must cover all 14 types, got {len(change_types)}")
    if len(expected) < 16:
    ...
    has_shifted_day_case = any(
        any(marker in case["text"] for marker in SHIFTED_DAY_MARKERS)
        and not any(c["change_type"] == "clock_state" for c in case.get("changes", []))
        for case in cases
    )
    if not has_shifted_day_case:
        raise ValueError(
            "holdout requires at least one past/future-day time reference "
            "case with no clock change"
        )
    has_bare_hour_case = any(
        re.search(r"[零一二三四五六七八九十两\d]点[，。；]", case["text"]) is not None
        and any(
            c["change_type"] == "clock_state"
            and isinstance(c.get("after_value"), str)
            and c["after_value"].endswith(":00")
            for c in case.get("changes", [])
        )
        for case in cases
    )
    if not has_bare_hour_case:
        raise ValueError("holdout requires at least one bare-hour scene time case")
    has_ling_x_case = any(
        re.search(r"点零[一二三四五六七八九\d]分", case["text"]) is not None
        and any(
            c["change_type"] == "clock_state"
            and isinstance(c.get("after_value"), str)
            and re.search(r":0[1-9]$", c["after_value"]) is not None
            for c in case.get("changes", [])
        )
        for case in cases
    )
    if not has_ling_x_case:
        raise ValueError("holdout requires at least one zero-padded minute case")
    has_impossible_perception_case = any(
        ("背出" in case["text"] or "能背出" in case["text"])
        and ("从未" in case["text"] or "没有发" in case["text"])
        and any(
            c["change_type"] == "knowledge_state"
            and c["expected_validation"] == "invalid"
            and c.get("mechanism") == "missing_transmission_path"
            for c in case.get("changes", [])
        )
        for case in cases
    )
    if not has_impossible_perception_case:
        raise ValueError(
            "holdout requires at least one impossible-perception knowledge case"
        )
    has_valid_path_knowledge_case = any(
        ("打开" in case["text"] and "文件" in case["text"])
        and ("念" in case["text"] or "读" in case["text"] or "引" in case["text"])
        and any(
            c["change_type"] == "knowledge_state"
            and c["expected_validation"] == "valid"
            and c.get("mechanism") in {
    ...
    if not has_valid_path_knowledge_case:
        raise ValueError("holdout requires at least one valid-path knowledge case")
    has_ack_not_delivery_case = any(
        ("受理" in case["text"])
        and any(
            c["change_type"] == "resignation_acknowledgement"
            and c["expected_validation"] == "valid"
            for c in case.get("changes", [])
        )
        and not any(c["change_type"] == "resignation_delivery" for c in case.get("changes", []))
        for case in cases
    )
    if not has_ack_not_delivery_case:
        raise ValueError("holdout requires at least one acknowledgement-not-delivery case")
    has_multi_evidence_case = any(
        _has_cross_paragraph_multi_evidence(case) for case in cases
    )
    if not has_multi_evidence_case:
        raise ValueError("holdout requires at least one multi-evidence cross-paragraph case")
    has_transmission_knowledge_case = any(
        ("链接" in case["text"] or "发进" in case["text"] or "上传" in case["text"] or "发到" in case["text"])
        and ("看完" in case["text"] or "读完" in case["text"] or "念出" in case["text"] or "引用" in case["text"] or "回了一条" in case["text"])
        and any(
            c["change_type"] == "knowledge_state"
            and c["expected_validation"] == "valid"
            and c.get("mechanism") in {
    ...
    if not has_transmission_knowledge_case:
        raise ValueError("holdout requires at least one transmission+perception knowledge case")
    has_self_assumed_employment_case = any(
        ("自己" in case["text"] or "宣布" in case["text"])
        and ("不来了" in case["text"] or "不再来" in case["text"])
        and any(
            c["change_type"] == "employment_state"
            and c["expected_validation"] == "invalid"
            and c.get("mechanism") == "self_assumed_effective"
            for c in case.get("changes", [])
        )
        for case in cases
    )
    if not has_self_assumed_employment_case:
        raise ValueError("holdout requires at least one self-assumed employment case")
    if not (has_unsourced and has_clock and has_location and has_operation):
        raise ValueError("holdout must include unsourced_project_fact, clock_state, location_state and storefront_operation_state")
    ...
    return {
        ...
        "all_14_types_covered": len(change_types) == 14,
    }
...
def _assert_component_binding() -> None:
    if EXTRACTOR.name != "semantic_extractor_wr2c513r10.py":
        raise RuntimeError(f"holdout runner bound to wrong extractor: {EXTRACTOR.name}")
    if PROJECTOR.name != "semantic_projector_wr2c513r6.py":
        raise RuntimeError(f"holdout runner bound to wrong projector: {PROJECTOR.name}")
    if project.__module__ != "experiments.world_runtime_writer_canary.semantic_projector_wr2c513r6":
        raise RuntimeError(f"holdout runner imports wrong projector module: {project.__module__}")
...
def seal(output_dir: Path = RUNTIME) -> dict[str, Any]:
    ...
    lock = {
        "schema_version": "world-runtime-sealed-holdout-wr2c513r12-lock-v1",
        "experiment_id": "world-runtime-semantic-judgment-wr2c513r12-sealed-unseen-holdout",
...
def build(output_dir: Path = RUNTIME) -> dict[str, Any]:
    ...
    for ordinal, case in enumerate(cases, 1):
        ...
        sample_id = f"WR2C513R12H-{ordinal:02d}"
        samples.append({
            "sample_id": sample_id,
    ...
    manifest = {
        "schema_version": "world-runtime-sealed-holdout-wr2c513r12-manifest-v1",
        "experiment_id": "world-runtime-semantic-judgment-wr2c513r12-sealed-unseen-holdout",
        "partition_role": "sealed_unseen_holdout",
        "holdout_sha256": _sha256_file(HOLDOUT_PATH),
        ...
        "all_14_types_covered": len({c["change_type"] for c in expected}) == 14,
    ...
    _write_json(manifest_path, manifest)
    ledger = {
        "schema_version": "world-runtime-sealed-holdout-wr2c513r12-ledger-v1",
        "samples": {
            sample["sample_id"]: {
...
def audit(output_dir: Path = RUNTIME) -> dict[str, Any]:
    ...
    return {
        "schema_version": "world-runtime-sealed-holdout-wr2c513r12-preflight-v1",
        "ready": not issues,
        "issues": issues,
...
def preflight(
    ...
    expected = {
        "schema_version": "world-runtime-semantic-canary-wr2c513r12-sealed-holdout-external-authorization-v1",
        "authorized": True,
    ...
    return {
        "schema_version": "world-runtime-sealed-holdout-wr2c513r12-external-preflight-v1",
        "ready": not issues,
        "issues": issues,
...
def evaluate(output_dir: Path = RUNTIME) -> dict[str, Any]:
    ...
    result = {
        "schema_version": "world-runtime-sealed-holdout-wr2c513r12-evaluation-v1",
        "partition_role": "sealed_unseen_holdout",
        "sample_count": manifest["sample_count"],
...
def run_once(
    ...
    for sample in manifest["samples"]:
        ...
        try:
            response = client.chat_completion(
                ...
                prompt_name="world_runtime_semantic_judgment_wr2c513r12_holdout_v1",
    ...
    return {
        "schema_version": "world-runtime-sealed-holdout-wr2c513r12-external-result-v1",
