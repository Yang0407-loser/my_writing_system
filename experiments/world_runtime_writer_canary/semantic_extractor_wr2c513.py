"""WR2-C5.1.3 semantic judgment contract and parser.

Same judgment+evidence contract as WR2-C5.1.2, with type 6 clarified: a text
containing both a pour action and a cleaned-and-stored action produces two
object_state changes.
...
from experiments.world_runtime_writer_canary.semantic_projector_wr2c513 import (
    build_delta,
    project,
)


SEMANTIC_EXTRACTOR_VERSION = "world-runtime-semantic-extractor-wr2c513-v1"
MAX_JUDGMENTS = 32
...
class SemanticExtractionArtifactC513(FrozenModel):
...
def parse_semantic_response(
    *,
...
) -> SemanticExtractionArtifactC513:
    ...
    try:
        ...
        raise ValueError("wr2c513_invalid_judgment_response") from exc
    ...
    if delta.output_hash != output_hash:
        raise RuntimeError("wr2c513_output_hash_mismatch")
    return SemanticExtractionArtifactC513(
        sample_id=sample_id,
