...
WR2-C7 adds chain normalization: the projector declares every change's
before_value relative to the state the commit starts from (chained mode), while
the committer requires each change's declared before to match the state at
application time.  This adapter rewrites accepted changes into chain-consistent
before values using the original base state and the commit-start state, and
rejects declared values that match none of: working state, commit-start state,
or original base state (integrity check).  When a chained
...
ADAPTER_VERSION = "world-runtime-state-commit-adapter-wr2c7-v1"
...
def _rewrite_chain_before(
    ...
    *,
    ...
    start = _state_by_key(before_state)
    ...
    for change in changes:
        ...
        if current is not None:
            ...
            allowed = (
                declared == effective
                or (key in base and declared == base[key])
                or (key in start and declared == start[key])
            )
            if not allowed and key not in base and key not in start:
