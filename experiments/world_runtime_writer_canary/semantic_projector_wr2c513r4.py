"""WR2-C5.1.3-R4 deterministic projector.
...
PROJECTOR_VERSION = "world-runtime-projector-wr2c513r4-v1"
...
def _resolve_judgment(
    ...
    if change_type == "clock_state":
        if _is_shifted_time_reference(evidence_text):
            return None
    ...
    *,
    ...
    for sequence, event in enumerate(events, 1):
        ...
        for evidence_index, (evidence, span) in enumerate(event.evidence, 1):
            evidence_id = f"ev:wr2c513r4:{sample_id.lower()}:{sequence}:{evidence_index}"
        ...
        changes.append(ProposedChangeV5(
            change_id=f"change:wr2c513r4:{sample_id.lower()}:{sequence}",
            sequence=sequence,
            change_type=event.change_type,
    ...
    return ProposedTypedDeltaV5(
        delta_id=f"delta:wr2c513r4:{sample_id.lower()}",
        sample_id=sample_id,
        scene_id=scene_id,
