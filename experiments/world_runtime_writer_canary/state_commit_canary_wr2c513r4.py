"""WR2-C5.1.3-R4 State Commit Canary Phase C2.1-R3: limited real-generation canary.
...
ADAPTER = ROOT / "experiments/world_runtime_writer_canary/state_commit_adapter_wr2c6.py"
RUNTIME = ROOT / ".world_runtime_state_commit_canary_runtime" / "c21r3"
...
def _assert_component_binding() -> None:
    if SOURCE.name != "state_commit_canary_wr2c513r4.py":
    ...
    if PROJECTOR.name != "semantic_projector_wr2c513r4.py":
        raise RuntimeError(f"canary runner bound to wrong projector: {PROJECTOR.name}")
    if ADAPTER.name != "state_commit_adapter_wr2c6.py":
...
def build(output_dir: Path = RUNTIME) -> dict[str, Any]:
    ...
    manifest = {
        "schema_version": "world-runtime-state-commit-canary-c21r3-manifest-v1",
        "experiment_id": "world-runtime-state-commit-canary-c21r3",
    ...
    ledger = {
        "schema_version": "world-runtime-state-commit-canary-c21r3-ledger-v1",
        "samples": {
            sample["subsection"]: {
...
def audit(output_dir: Path = RUNTIME) -> dict[str, Any]:
    ...
    return {
        "schema_version": "world-runtime-state-commit-canary-c21r3-preflight-v1",
        "ready": not issues,
        "issues": issues,
        "pending": pending,
...
def preflight(
    ...
    return {
        "schema_version": "world-runtime-state-commit-canary-c21r3-external-preflight-v1",
        "ready": not issues,
        "issues": issues,
...
def run_once(
    ...
    if not check["ready"]:
        raise RuntimeError("c21r3_preflight_failed:" + "|".join(check["issues"]))
    manifest = _read_json(output_dir / "private/locked-manifest.json")
    ledger_path = output_dir / "attempt-ledger.json"
    ...
    previous_text = ""
    current_state = gold.state_before
    for sample in manifest["samples"]:
        subsection = sample["subsection"]
        entry = ledger["samples"][str(subsection)]
        if entry["status"] != "pending" or entry["attempt_count"] != 0:
            raise RuntimeError(f"c21r3_refusing_nonpristine_subsection:{subsection}")
        ...
        try:
            ...
            final_text = client.chat_completion(
                ...
                prompt_name="state_commit_canary_c21r3_generation_v1",
            ...
            judgment = client.chat_completion(
                ...
                prompt_name="state_commit_canary_c21r3_judgment_v1",
            ...
            artifact = parse_semantic_response(
                ...
                sample_id=f"C21R3-S{subsection}",
                ...
                record.update(
                    ...
                    created_facts=[
                        ...
                        if entry.fact_id and entry.fact_id not in {
                            fact.fact_id for fact in current_state.facts
                        }
                    ],
    ...
    report = {
        "schema_version": "world-runtime-state-commit-canary-c21r3-report-v1",
        "partition_role": "limited_real_canary",
        "summary": summary,
        "records": records,
