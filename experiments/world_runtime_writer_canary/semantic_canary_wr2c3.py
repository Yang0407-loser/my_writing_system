"""WR2-C3 Development-v3 semantic judgment canary builder and evaluator."""
...
from experiments.world_runtime_writer_canary.semantic_projector_wr2c3 import project


ROOT = Path(__file__).resolve().parents[2]
SOURCE = Path(__file__).resolve()
EXTRACTOR = ROOT / "experiments/world_runtime_writer_canary/semantic_extractor_wr2c3.py"
PROJECTOR = ROOT / "experiments/world_runtime_writer_canary/semantic_projector_wr2c3.py"
VALIDATOR = ROOT / "experiments/world_runtime_writer_canary/delta_shadow_wr2b.py"
FIXTURE = ROOT / "experiments/world_runtime_writer_canary/fixtures/wr2c3_semantic_development_v3.json"
EXTERNAL_RUNNER = ROOT / "experiments/world_runtime_writer_canary/external_runner_wr2c3.py"
DEFAULT_OUTPUT = ROOT / ".world_runtime_wr2c3_semantic_canary_runtime"
...
def _projector_roundtrip(case: dict[str, Any]) -> None:
    ...
    if actual != expected:
        raise ValueError(
            f"WR2-C3 projector roundtrip mismatch in {case['case_id']}: "
...
def build(output_dir: Path = DEFAULT_OUTPUT) -> dict[str, Any]:
    if (output_dir / "attempt-ledger.json").exists():
        raise FileExistsError("WR2-C3 attempt ledger exists; refusing rebuild")
    if not EXTERNAL_RUNNER.exists():
        raise FileNotFoundError("WR2-C3 external runner missing")
    ...
    if len(samples) != GATES["sample_count"] or len(change_types) != 13:
        raise ValueError("WR2-C3 sample or ontology coverage mismatch")
    manifest = {
        "schema_version": "world-runtime-semantic-canary-wr2c3-manifest-v1",
        "experiment_id": "world-runtime-semantic-judgment-wr2c3-development-v3",
        "partition_role": "visible_development_v3_not_holdout",
    ...
    ledger = {
        "schema_version": "world-runtime-semantic-canary-wr2c3-ledger-v1",
        "samples": {
            sample["sample_id"]: {
...
def _assert_integrity(output_dir: Path) -> dict[str, Any]:
    ...
    for field, expected in checks.items():
        if manifest.get(field) != expected:
            raise RuntimeError(f"wr2c3_frozen_drift:{field}")
    return manifest
...
def audit(output_dir: Path = DEFAULT_OUTPUT) -> dict[str, Any]:
    ...
    result = {
        "schema_version": "world-runtime-semantic-canary-wr2c3-preflight-v1",
...
def evaluate(output_dir: Path = DEFAULT_OUTPUT) -> dict[str, Any]:
    ...
    for sample in manifest["samples"]:
        ...
        if entry["status"] != "succeeded" or entry["attempt_count"] != 1:
            raise RuntimeError("WR2-C3 requires exactly one succeeded attempt per sample")
        ...
        if hashlib.sha256(response_text.encode("utf-8")).hexdigest() != entry["output_sha256"]:
            raise RuntimeError("WR2-C3 response hash mismatch")
    ...
    result = {
        "schema_version": "world-runtime-semantic-canary-wr2c3-evaluation-v1",
        "partition_role": "visible_development_v3_not_holdout",
        "sample_count": manifest["sample_count"],
        ...
        "state_mutations": 0,
        "commits": 0,
        "decision": "development_v3_passed_new_unseen_holdout_required" if passed else "hold_semantic_development_v3_failed",
        "items": items,
    }
