"""WR2-C5 deterministic projector.

Same boundary as WR2-C4 (model judges, projector owns canonical fields) plus
``storefront_operation_state`` support for explicit store opening/closing.
...
PROJECTOR_VERSION = "world-runtime-projector-wr2c5-v1"
    ...
    *,
    ...
    for sequence, event in enumerate(events, 1):
        ...
        for evidence_index, (evidence, span) in enumerate(event.evidence, 1):
            evidence_id = f"ev:wr2c5:{sample_id.lower()}:{event.judgment_index + 1}:{evidence_index}"
        ...
        changes.append(ProposedChangeV5(
            change_id=f"change:wr2c5:{sample_id.lower()}:{sequence}",
            sequence=sequence,
    ...
    return ProposedTypedDeltaV5(
        delta_id=f"delta:wr2c5:{sample_id.lower()}",
        sample_id=sample_id,
        scene_id=scene_id,
