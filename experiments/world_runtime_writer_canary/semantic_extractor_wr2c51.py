"""WR2-C5.1 semantic judgment contract and parser.

Same judgment+evidence contract as WR2-C5, with two prompt refinements:

1. Type 2 clarifies that a handoff inside a paid sale is not a separate
   free handoff.
2. Type 5 strengthens unsourced-entity detection for narrative-introduced
   roles (while keeping fictional drafts out).

Canonical fields remain fully owned by the WR2-C5.1 projector.
"""
...
from experiments.world_runtime_writer_canary.semantic_projector_wr2c51 import (
    build_delta,
    project,
)


SEMANTIC_EXTRACTOR_VERSION = "world-runtime-semantic-extractor-wr2c51-v1"
MAX_JUDGMENTS = 32
...
class SemanticExtractionArtifactC51(FrozenModel):
...
def parse_semantic_response(
    *,
    ...
    try:
        ...
        raise ValueError("wr2c51_invalid_judgment_response") from exc
    ...
    if delta.output_hash != output_hash:
        raise RuntimeError("wr2c51_output_hash_mismatch")
    return SemanticExtractionArtifactC51(
        sample_id=sample_id,
