"""WR2-C6 typed-delta validator with working-state chaining.
...
CONTRACT_VERSION = "world-runtime-typed-delta-shadow-wr2c6-v1"
...
class ShadowValidationV5(FrozenModel):
    ...
    downstream_consumer: Literal["wr2c6_development_audit"] = "wr2c6_development_audit"
    schema_version: Literal[CONTRACT_VERSION] = CONTRACT_VERSION
...
def validate_delta_v6(
    delta: ProposedTypedDeltaV5,
    *,
    state=None,
) -> ShadowValidationV5:
    """Validate a typed delta against an explicit base state (chained mode)."""
    if state is None:
        _, states, _ = wr1r._artifacts()
        state = states[delta.state_variant]
    if delta.base_revision != state.revision:
        raise ValueError("WR2-C6 validator base revision mismatch")
    rules = {
    ...
    for change in delta.changes:
        if not set(change.evidence_ids).issubset(evidence_ids):
            raise ValueError("WR2-C6 validator evidence reference mismatch")
        ...
        if change.change_type in {"storefront_public_sale", "storefront_public_handoff"}:
            ...
            raise ValueError(f"unsupported WR2-C6 change type: {change.change_type}")
        items.append(item)
    ...
    return ShadowValidationV5(
        validation_id=f"validation:wr2c6:{delta.sample_id}",
