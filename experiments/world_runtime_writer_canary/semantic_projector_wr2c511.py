"""WR2-C5.1.1 deterministic projector.

Same boundary as WR2-C5.1 (model judges, projector owns canonical fields) plus:

1. Deterministic clock fallback: when the model did not judge a clock change
   but the text contains an explicit, parseable time expression later than the
   state clock (outside quotes), the projector synthesizes an evidence-bound
   clock change.  Explicit surface time is a local projection decision.
2. Knowledge perceiver selection by distance to the perception verb, so a
   merged evidence excerpt with both sender and perceiver picks the perceiver.
...
PROJECTOR_VERSION = "world-runtime-projector-wr2c511-v1"
...
def _fallback_clock_event(
    *,
    text: str,
    state,
    judgments: list[dict[str, Any]],
) -> ProjectedEvent | None:
    """Synthesize a clock change from an explicit narrative time expression."""
    ...
    for pattern in _CLOCK_PATTERNS:
        for match in pattern.finditer(text):
            ...
            if best is None or parsed > best[0]:
                best = (parsed, match.start(), match.end(), match.group(0))
    if best is None:
        return None
    parsed, start, end, excerpt = best
    evidence = RawEvidence(excerpt=excerpt, occurrence=1)
...
def project(
    *,
    ...
    if not any(event.change_type == "clock_state" for event in events):
        fallback = _fallback_clock_event(text=text, state=state, judgments=judgments)
    ...
    *,
    ...
    for sequence, event in enumerate(events, 1):
        ...
        for evidence_index, (evidence, span) in enumerate(event.evidence, 1):
            evidence_id = f"ev:wr2c511:{sample_id.lower()}:{event.judgment_index + 1}:{evidence_index}"
        ...
        changes.append(ProposedChangeV5(
            change_id=f"change:wr2c511:{sample_id.lower()}:{sequence}",
            sequence=sequence,
            change_type=event.change_type,
    ...
    return ProposedTypedDeltaV5(
        delta_id=f"delta:wr2c511:{sample_id.lower()}",
        sample_id=sample_id,
        scene_id=scene_id,
