"""WR2-C5.1.3 deterministic projector.

Same boundary as WR2-C5.1.2 (model judges, projector owns canonical fields) plus:

1. Clock events are ordered by time (not judgment order).
2. A ``publication_state`` change is subsumed by a ``repeated_completed_event``
   in the same delta.
3. Local storefront-open fallback: explicit door-opening tokens synthesize a
   ``storefront_operation_state`` change when the model missed it.
...
PROJECTOR_VERSION = "world-runtime-projector-wr2c513-v1"
    ...
    *,
    ...
    for sequence, event in enumerate(events, 1):
        ...
        for evidence_index, (evidence, span) in enumerate(event.evidence, 1):
            evidence_id = f"ev:wr2c513:{sample_id.lower()}:{sequence}:{evidence_index}"
        ...
        changes.append(ProposedChangeV5(
            change_id=f"change:wr2c513:{sample_id.lower()}:{sequence}",
            sequence=sequence,
            change_type=event.change_type,
    ...
    return ProposedTypedDeltaV5(
        delta_id=f"delta:wr2c513:{sample_id.lower()}",
        sample_id=sample_id,
        scene_id=scene_id,
