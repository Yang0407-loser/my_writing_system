...
def validate_runtime(queue: list[dict[str, Any]]) -> None:
    ...
    for item in queue:
        ...
        if settings.LLM_BASE_URL != spec["base_url"]:
            raise ValueError("runtime base URL differs from probe contract")
        if settings.LLM_MODEL != spec["model"]:
            raise ValueError("runtime model differs from probe contract")
...
def reserve(ledger: Path, item: dict[str, Any]) -> None:
    with closing(sqlite3.connect(ledger, isolation_level=None)) as db:
        ...
        row = db.execute(
            "SELECT request_sha256,status,attempt_count FROM generation_queue WHERE generation_id=?",
            (item["generation_id"],),
        ).fetchone()
        ...
        db.execute(
            "UPDATE generation_queue SET status='reserved',attempt_count=1 WHERE generation_id=?",
            (item["generation_id"],),
        )
        db.execute(
            "INSERT INTO generation_attempts(generation_id,attempt_number,request_attempted) VALUES(?,1,0)",
            (item["generation_id"],),
        )
        db.execute("COMMIT")
...
def mark_attempted(ledger: Path, generation_id: str) -> None:
    with closing(sqlite3.connect(ledger)) as db, db:
        changed = db.execute(
            "UPDATE generation_attempts SET request_attempted=1 WHERE generation_id=? AND request_attempted=0",
            (generation_id,),
        ).rowcount
        if changed != 1:
...
def finalize(ledger: Path, generation_id: str, outcome: str, **values: Any) -> None:
    with closing(sqlite3.connect(ledger, isolation_level=None)) as db:
        db.execute("BEGIN IMMEDIATE")
        state = db.execute(
        ...
        attempt = db.execute(
            "SELECT request_attempted,outcome FROM generation_attempts WHERE generation_id=?",
            (generation_id,),
        ).fetchone()
        if state != ("reserved", 1) or attempt != (1, None):
            db.execute("ROLLBACK")
            raise GenerationAlreadyAttemptedError(generation_id)
        db.execute("UPDATE generation_queue SET status=? WHERE generation_id=?", (outcome, generation_id))
        db.execute(
            "UPDATE generation_attempts SET outcome=?,response_sha256=?,finish_reason=?,"
            "input_tokens=?,output_tokens=?,latency_seconds=?,error_type=?,error_message_sha256=? "
            "WHERE generation_id=?",
            (
                outcome,
...
def basic_text_checks(text: str, finish_reason: str | None) -> dict[str, Any]:
    stripped = text.strip()
    leaked_labels = (
        "fixed_scene_contract", "narrative_task", "must_happen", "forbidden_events",
        "allowed_end_state", "传统文学叙事场景", "商业网文叙事场景",
    )
    return {
        "nonempty": bool(stripped),
        "character_count": len(stripped),
        "within_target_band_1000_1500": 1000 <= len(stripped) <= 1500,
        ...
        "field_or_mode_leakage_detected": any(label in stripped for label in leaked_labels),
    }
...
def execute_all(output_dir: Path = DEFAULT_OUTPUT) -> dict[str, Any]:
    ...
    for item in sorted(queue, key=lambda value: value["ordinal"]):
        ...
        try:
            ...
            content = client.chat_completion(
                ...
                max_retries=0,
                json_mode=False,
                prompt_name="style_root_cause_probe_v0",
            ...
            write_json(
                ...
                {
                    "schema_version": "style-root-cause-text-v0",
    ...
    summary = {
        "schema_version": "style-root-cause-run-summary-v0",
        "requested": len(queue),
