...
| 2026-07-22 | WriterExecutionContract真实Canary归因与V1.1收口 | 只读重建任务`019fc0c8…`四份合同；增加超预算attempted token/分项账本、严格完全重复渲染去重、全文长度及末尾停止说明；不改typed语义、450上限、Writer Prompt主体或生成链 | V1真实注入2/4，正文1256/2272/1592/3095字；V1.1合同888/579/496/339 token，仅S1.4可注入；S1.3/S1.4语义hash保持，source/hash追溯100%；确认description多处重复、完整下一节边界、每段约400字与1000字总目标及4-5事件存在竞争；定向测试28 passed、compileall通过 | 状态=`v11_engineering_complete_not_eligible_for_demo`；生产保持off，不再运行同配置Demo；唯一下一方向为独立outline事件密度审计，不启动Validator/Repair/Phase5/6 |
| 2026-07-22 | Outline Budget Advisor V1 | 新增只读确定性事件单元、provisional篇幅公式、章节最大余数守恒分配、task-scoped建议API和作者显式应用UI；不读取旧50维风格字段，不调用Writer/LLM，不修改生成链 | 固定任务四节均给出建议；事件数5/3/3/4，S1.2识别两次跨周推进，S1.3复杂度不高于S1.2，S1.4宽泛要点降为medium；preferred合计3550但章节分配严格守恒4000；15/15事件来源可追溯；检出段落约400字与小节全文1000字潜在冲突 | 状态=`production_read_only_advisory_ready`；公式不是质量真值，作者点击后才改变前端target_words且仍需手动保存；WriterExecutionContract继续off；下一步最多在一个真实outline上由作者使用建议，不自动启动Writer或新实验 |
| 2026-07-24 | SceneSpec → BoundaryValidator 真实工件贯通 | 修复生成前 typed SceneSpec 在 application 后丢失、post-commit Runner 无可用工件的断链；同一小节对象直接传递，显式工件优先且保留旧 provider 兼容路径；不二次编译、不调用LLM | 四个 SceneSpec golden hash和R5冻结预测hash保持；off/shadow messages、Writer输出/调用、StateCommitter/checkpoint不变；定向51 passed、compileall通过；真实Validator样本0 | 状态=`engineering_handoff_complete_real_shadow_pending`；默认SceneSpec off、Validator false、ExecutionContract off；下一步只允许一个自然真实shadow任务，不授权Repair、阻断、自动重写、Phase5/6 |
| 2026-07-24 | SceneSpec → BoundaryValidator唯一真实shadow收口 | 自然任务`c3fd7953…`的4个小节均构建一次SceneSpec并以同一typed工件在commit后交付Validator；不注入Writer、不增加调用、不影响任务完成 | 4/4=`explicit_artifact`，provider unavailable=0，shadow error=0，Writer主调用=4，Mandatory重试=0；结果2 pass/1 fail/1 skip，唯一告警把S1.2明确要求的拍摄动作误判为Q8式未来越界；两个pass无required-event结果 | 工件贯通=`passed`，BoundaryValidator V1=`not_promoted`；默认继续off，关闭该生产路线，不扩关键词/阈值/样本，不做UI警告、Repair、阻断或自动重写 |
| 2026-07-24 | OutlineEventContract V1 | 新增冻结 typed 大纲事实层、Legacy 确定性适配、作者确认/增量失效和当前/下一小节边界；Budget Advisor 改为只消费 contract；确认事件与应用字数在原弹窗中分离 | 固定任务4/4 contract、16/16事件追溯、重复ID=0、hash确定性100%；S1.2两次跨周且拍摄=`current`，S1.3事件作为deferred，复杂度低于S1.2；章节4000字严格守恒；Writer/LLM调用0 | 状态=`typed_outline_fact_layer_ready`；Legacy结果仅proposed且不自动hard；Writer、SceneSpec、Validator、StateFrame、Mandatory Event均未接入；停止并等待用户另行授权CharacterArcProjection |
| 2026-07-24 | CharacterArcProjection V1 | 新增从typed事件到每角色候选的离线确定性投影；区分plot/texture/soft/hard建议，hard必须补齐before/trigger/after/evidence/rationale；支持稳定ID、stale/superseded和V2兼容导出，不建立隐式边 | 固定真实任务候选8、排除9、追溯100%、重复ID=0；ordinary=6、texture=1、unsupported=1，authoritative=0、hard=0；定向测试不调用Writer/LLM | 状态=`offline_projection_ready_not_promoted`；真实事件仍proposed且代词无法可靠绑定角色，因此不接Writer/EventGraph、不切V2；下一步仅可另行授权小规模作者确认UI或contract-grounded Planner shadow |
| 2026-07-25 | 角色弧小规模确认 UI | 在现有大纲页增加“弧线”薄弹窗，只展示 decision/state-transition；作者可标记 soft/hard/非角色弧，hard 必须填写五项状态转变依据；篇幅和弧线入口均立即显示弹窗；修复旧API模块缓存，并在事件结构中增加作者显式人物归属，仍不自动猜代词 | 第一小节已确认5事件中两条弧线候选因“她”导致actor为空的问题已定位；作者actor覆盖可规范化保存；事件契约/持久化/投影/API/相邻入口定向32项通过，JS语法和compileall通过，Writer/LLM调用0 | 状态=`thin_review_ui_ready_not_promoted`；确认结果仍不接 CharacterManager、Writer、EventGraph 或 checkpoint；作者需给代词事件明确勾选角色并重新确认事件结构，再在弧线弹窗审阅 |
| 2026-07-25 | StateFrame V1 小节状态快照与三维质量基线 | 保留旧 StateFrameCompiler/SceneSpec 兼容语义，新增只读 StateFact、StateExpectation、before/after Frame、StateDelta、三维 QualityObservation 和只读 API；交接、人物/关系状态、伏笔继续为现有权威来源；不注入 Writer、不调用 LLM | 最新完成真实任务4小节、8个Frame均为partial；必要来源覆盖3/7=42.86%，进入Frame的事实source/hash追溯率100%；2个状态转变预期及三项质量指标因缺小节历史工件保持unassessable；定向测试34 passed、compileall通过 | 状态=`read_only_contract_ready_upstream_history_gap`；发现每小节before/after checkpoint与typed bundle未持久化的可观测性断链，不能用最终状态倒推历史；暂不建议Writer shadow注入，下一步若授权只能先修复单一小节快照持久化缺口，不同时重构三个提取器 |
| 2026-07-25 | StateFrame V1 小节级快照持久化修复 | 在每小节 Writer 调用前冻结 Before，在正文 commit 与既有 post-commit observers 后、节尾状态更新前冻结 After 并计算 Delta；同一逻辑工件依次镜像到 Blackboard、checkpoint 和 TaskStore `analysis_json`，不新增数据库结构、不注入 Writer、不调用 LLM | 四小节 fixture 的 Before/After/Delta 均 4/4，正式记录重复 0，TaskStore 与模拟 Redis 丢失恢复均 100%，已持久化事实 source/hash 追溯 100%；旧任务无原生历史时显式 unavailable 或 partial reconstruction；节尾 Character State/handover 不回填早期小节 | 状态为 `engineering_complete_real_demo_pending`；只解决历史工件持久化，不提高三项质量规则或上游数据质量，不授权精简 legacy 上下文。下一步最多允许另行授权一个真实四小节 Demo 验证落盘与重启恢复，不自动注入 Writer |
| 2026-07-25 | StateFrame V1 唯一真实四小节持久化验收 | 固定真实任务 `6e52740c-c959-4c84-8651-b46ceebfd88b`，只读核对 Blackboard、checkpoint、TaskStore、捕获顺序、Worker 重启和隔离 Redis 空数据回退；不重新生成、不注入 Writer、不修改质量规则 | 任务 completed；Before/After/Delta=4/4/4，pending/error/重复记录均0；三层 ID/hash 全等；Worker 重启后 hash 稳定；TaskStore 回退4/4且 `source=task_history`、`reconstructed=false`；96/96 facts 具有 source/hash，未增加 LLM 调用 | 状态=`accepted`，小节级持久化正式通过真实验收；三项质量中 handover/foreshadow 仍 unassessable、人物状态仅 partial，不把 unavailable 计作质量失败；StateFrame 仍未注入 Writer，不自动开始下一项优化 |
| 2026-07-25 | SubsectionOutcomeBundle V1 只读覆盖审计 | 新增不具权威写入能力的 typed Bundle/Adapter，仅从固定真实任务的 TaskStore、StateFrame history、关系/伏笔/事件库读取已有工件；严格区分 subsection exact、section aggregate、current store 和 task-final snapshot；不修改 Writer/Coordinator/StateFrame/存储，不调用 LLM | 4/4 Bundle 可确定性生成，重复ID=0；20个 component 中 available/partial/unavailable/conflicted=0/4/16/0，五类 subsection-exact 覆盖均0%；23/23实际 source refs 可追溯；发现节级 handover、最终人物状态、伏笔当前快照和 subsection=0 经历事件均不能充当小节 delta | 状态=`adapter_ready_existing_assets_insufficient_for_shadow_hook`；适配器机械门槛通过，但 shadow hook 没有新增事实价值，暂不建议接入；handover/foreshadow仍不可评估、人物状态仅partial，不自动开始PostWriteOrchestrator或StateFrame注入 |
| 2026-07-25 | Subsection Handover Artifact V1 | 在既有每小节 handover 提取结果之后增加失败隔离的持久化 sidecar；正式记录仅在正文 commit、既有 post-commit observers 和 StateFrame After 之后生成，并镜像到 Blackboard、checkpoint 与 TaskStore `analysis_json`；不修改 Prompt、返回 note、节级汇总或消费者 | synthetic 门槛通过；真实四小节任务完成，4/4 records、pending=0、errors=0、重复ID=0；Blackboard/checkpoint/TaskStore 完全一致；TaskStore 只读回退成功；任务完成后重启 Worker，record/output/prompt/handover hash 与幂等键全部稳定；新增 Writer/LLM 调用0 | 状态=`real_demo_accepted`；验收仅证明小节级交接工件可持久化和恢复，不代表交接内容质量已成为真值，也不授权 StateFrame/Bundle 注入、Validator、Repair 或其他后处理扩展 |
| 2026-07-25 | Subsection Handover V1 内容有效性验收 | 对同一真实四小节任务按 A预期承接、B原子claim忠实度、C下一小节承接三阶段分别冻结；只读审计4条原生handover和3个过渡，不重新生成、不调用Writer/外部LLM、不修改生产 | sidecar同章内实际注入0/3；74 claims中supported/partial/unsupported/ambiguous/unverifiable=34/7/3/15/15，严格Precision=77.27%；critical承接4/9、supporting承接3/7；3个过渡正确1、连续性错误2，但均不可归因于未被消费的handover；source/hash与evidence span追溯100% | 状态=`persistence_accepted_content_not_validated`；handover continuity仅`partially_assessable_native_artifact_not_quality_truth`，不晋级下游消费；唯一下一步是最小extractor契约修复：仅输出带source span的节尾状态、未完成事件和下一场景边界，禁止无证据心理推断及无milestone来源的arc状态 |
| 2026-07-25 | Subsection Handover Contract V2 最小修复 | 新增 evidence-backed typed contract、本地逐条验证、确定性 outline boundary 与 legacy note 适配；版本配置默认 `v1`，V2 仍复用每小节现有一次提取调用；持久化仅增加可选 V2 元数据，不修改旧消费者、正文 Prompt、提交顺序或数据库结构 | 69 项定向测试全部通过，受影响模块 compileall 通过；sealed V1 负面回归确认 3 条无依据心理推断、1 条 stale fact、15 条无 milestone 来源 arc pending 均被 V2 契约策略拦截；4 个真实 outline boundary 可构建，2 个既有边界风险保留为记录；新增 Writer/LLM 调用 0 | 状态 `engineering_gate_passed_real_demo_authorized`；生产默认继续 V1。只允许另行执行一次真实四小节 V2 Demo，Demo 完成前不得将 V2 设为默认或开始其他下游优化 |
| 2026-07-25 | Subsection Handover Contract V2 唯一真实 Demo 收口 | 固定真实任务 `cd830826-61b0-4840-b2a7-45cf807599e0`，核对4个小节的提取调用、持久化 sidecar、任务完成与 fail-open；不读取或提交正文，不修改 V2、Writer 或下游消费者 | 任务 completed、正文主调用4、Mandatory Event实际重试0；4/4 Handover调用均在600输出token处以`finish=length`截断，随后以`ValueError`失败；合计12,361 known tokens、约24.8秒，typed contract=0、handover chain=0；4条boundary hash和source manifest仍持久化，正文/checkpoint/Review未受影响 | 状态 `real_demo_failed_output_truncation`；工程隔离通过但真实内容门槛不可评估，V2不晋级且默认继续V1；禁止同配置重跑，若另行授权只能先紧凑化V2输出契约并离线证明可容纳，再决定是否值得重新验证 |
| 2026-07-25 | Subsection Handover Contract V2.1 紧凑输出契约 | 新增source registry与紧凑`v/s/o/f/a`传输层；模型只返回索引、span、短枚举及16字结构化语义，source ID/hash/excerpt、milestone和boundary均由本地恢复；复用V2 validator与legacy adapter，默认继续V1 | V2代表性典型/最坏输出约1371/3030 estimated tokens；V2.1典型127、最坏合法444，600上限下余量156；首次24字设计579被测试拒绝；定向测试60 passed、sealed失败类别与持久化兼容通过、compileall通过，真实Writer/LLM调用0 | 状态 `engineering_gate_passed_one_v21_demo_authorized`；只恢复唯一一次V2.1真实四小节Demo的工程条件，不宣称生成质量通过、不自动运行Demo、不接StateFrame/OutcomeBundle、不切生产 |
