...
| 2026-07-21 | Writer Condense可控真实验证接入 | 固定`cd340fcc…`审计3次二次压缩；新增默认legacy、可选warn的`WRITER_CONDENSE_MODE`，warn只记录超长并保留完整初稿，不构造condense请求 | 4小节目标均1,000字，初稿7,044字、压缩后6,185字；3次调用耗费8,998 known token/46.1秒，仅删除859字（10.475 token/字）；同形调用理论22→19；handover与部分状态在压缩前提取的顺序风险已登记但未修改 | 工程接入完成，生产默认仍legacy，尚无warn真实正文；只允许1个正常四小节任务判断总长度与正文可用性，不做阈值网格，不启动handover缓存或shared extraction |
