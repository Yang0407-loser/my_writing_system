...
> 状态：Phase 3 Batch 2A 离线消融已完成，等待 2 条新增候选的定向人工复核；继续 shadow，生产 Writer 仍使用旧检索，Phase 4 未开始
> 日期：2026-07-18  
> 执行者：Codex  
...
| 2026-07-18 | Phase 3 Batch 2A 离线诊断消融 | 对冻结 trace 和 38 条人工标签复跑 10,368 个意图、角色分、阈值、query/result 上限、同章惩罚及 token 预算组合；用按 query 留一法检查过拟合；未调用 LLM、未改生产代码或默认配置 | 基线闭集 P=55.26%；去 scene 配置 P=63.64%、已知相关保留=100%、新增未标注=2；收紧到去 character/max3 配置 P=66.67%、保留=76.19%；留一聚合 P=60.61%、保留=95.24%；新增定向复核项=2；unit=141/0、integration=8/0、quality=19/0 | 收紧宽泛意图能减少误召回，但尚无稳健配置达到 68% 门槛；等待 2 条新增候选人工复核，继续 shadow，不调权重、不切生产、不开始 Phase 4 |
