...
| 2026-07-24 | 角色弧小规模确认 UI | 在现有大纲页增加“弧线”薄弹窗，只展示 decision/state-transition；作者可标记 soft/hard/非角色弧，hard 必须填写五项状态转变依据；确认前重新校验事件确认状态与来源 hash；篇幅和弧线入口均改为点击后立即显示总览弹窗及加载状态 | preview 不写状态；confirm 只写独立 `character_arc_projection_review` 工件；Blackboard JSON 往返、投影/API/相邻篇幅入口定向测试通过，JS语法和compileall通过，Writer/LLM调用0 | 状态=`thin_review_ui_ready_not_promoted`；不接 CharacterManager、Writer、EventGraph 或 checkpoint；下一步只在一个真实大纲上做少量作者确认，先验证字段是否可用，不自动启动Planner shadow |
