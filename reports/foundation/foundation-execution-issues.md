...
| FND-008 | Task 6 Docker dependency smoke | The Docker dependency image cannot be built because the local Docker Desktop Linux daemon is not running. | `docker build` cannot open `npipe:////./pipe/dockerDesktopLinuxEngine`; the Docker CLI itself is version 29.5.3. | Deferred to central remediation; retry when daemon is available |
