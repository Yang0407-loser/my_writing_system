...
| FND-008 | Task 6 Docker smoke / Task 10 PostgreSQL gate | Docker image build and the real PostgreSQL gate cannot run because the local Docker Desktop Linux daemon is not running. | `docker build` and `docker compose up -d postgres` cannot open `npipe:////./pipe/dockerDesktopLinuxEngine`; Compose syntax validates, and the canonical suite reports `34 passed, 2 skipped` with both PostgreSQL tests explicitly skipped. | Deferred to central remediation; retry build and require PostgreSQL tests to pass when daemon is available |
