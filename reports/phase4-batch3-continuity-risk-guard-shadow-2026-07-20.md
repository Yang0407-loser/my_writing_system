...
按预设停止规则，Phase 4 的“完整旧小节整项选择”路线判定失败。本批不调用 Writer LLM、不做 A/C 生成验证、不切生产输入，也不开始 Phase 5。下一候选只能是另行授权、可追溯的节级摘要实验。
...
自然叙事中的“周六、凌晨、五天前、今早”等时间锚点非常普遍。在“出现任一风险信号就保留全文、无法排除风险也保留”的契约下，Guard 无法安全删除任何较早小节。这不是通过调阈值可以解决的问题，而是完整小节粒度与保守连续性要求之间的结构性冲突。
...
若继续 Phase 4，应另行授权测试“可追溯节级摘要”，并把它视为全新的有损压缩假设，而不是继续调 Broker：
