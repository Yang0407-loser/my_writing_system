...
Outline Budget Advisor V1 已完成只读生产接入。它在 Writer 调用前，从现有小节 description、key points、角色名单和四个主要风格旋钮中提取可追溯事件单元，给出篇幅范围、密度风险、结构建议和章节预算分配。默认只展示建议；只有作者点击“应用推荐值”才修改浏览器中的 `target_words`，之后仍需走现有保存流程。
...
- 新 API 仅分析请求中的未保存 outline 草稿，不读取或写入 Redis、数据库或 Chroma。
...
调用顺序为：作者编辑 outline → 点击“篇幅” → 只读 API 返回建议 → 前端展示 → 作者可选择“应用推荐值” → 作者确认后手动保存 → 后续才可开始写作。

## 固定任务结果

固定任务 `019fc0c8-1a20-4be3-8252-54aaf1c5aa27` 的章节预算为 4000 字。公式给出的四节 preferred 合计为 3550 字；章节最大余数分配仍严格守恒为 4000 字，不会静默扩大章节目标。

| 小节 | 当前 | 建议 min/preferred/max | 事件 | 时间跳转 | 场景变化 | 人物 | 密度 | 动作 | 置信度 |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | --- | --- | --- |
| S1.1 | 1000 | 1100 / 1300 / 1500 | 5 | 2 | 0 | 1 | dense | review_structure | medium |
| S1.2 | 1000 | 700 / 850 / 1000 | 3 | 2 | 0 | 3 | balanced | keep | high |
| S1.3 | 1000 | 550 / 650 / 750 | 3 | 0 | 1 | 2 | light | review_structure | high |
| S1.4 | 1000 | 650 / 750 / 850 | 4 | 0 | 0 | 0 | light | review_structure | medium |
...
- S1.4 四条 key points 都较宽泛，因此降低置信度并建议结构复核。
- 四节均检出 style brief 的“段落长度”与“小节全文目标”可能竞争；该文本没有参与公式。
- 15/15 事件单元都有稳定 unit ID、source ID 和 source hash。公开报告不保存原 outline 文本副本。
