...
真实页面复核又发现两项显示断链：浏览器缓存了旧 `api.js`，以及第一小节
的状态变化/决定使用“她”而没有 actor，导致严格投影不会把它绑定给林晚。
现已为 API 模块单独增加版本号，并在事件结构中增加作者显式“人物归属”
勾选。作者提交后，actor 会进入规范化事件契约；系统仍不会自动猜代词。
...
- 事件契约 actor 覆盖、持久化、角色弧投影、确认 API、Blackboard JSON
  往返和相邻篇幅建议入口：32 项通过；
